...
- Jobs persist under `~/.krabkrab/cron/` so restarts donâ€™t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.krabkrab/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `krabkrab cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, KrabKrab defaults to `announce`.
...
- Job store: `~/.krabkrab/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.krabkrab/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```bash
krabkrab cron run <jobId>
krabkrab cron run <jobId> --due
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`krabkrab system event`](/cli/system).

## Troubleshooting

### â€œNothing runsâ€

- Check cron is enabled: `cron.enabled` and `krabkrab_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- KrabKrab applies exponential retry backoff for recurring jobs after consecutive errors:
