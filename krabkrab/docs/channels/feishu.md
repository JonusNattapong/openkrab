...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects KrabKrab to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
krabkrab plugins install @krabkrab/feishu
```

Local checkout (when running from a git repo):

```bash
krabkrab plugins install ./extensions/feishu
```
...
If you just installed KrabKrab, run the wizard:
...
2. Configuring app credentials in KrabKrab
...
1. You already ran `krabkrab channels add` for Feishu
2. The gateway is running (`krabkrab gateway status`)
...
```bash
krabkrab pairing approve feishu <CODE>
```
...
2. Run `krabkrab logs --follow` and look for `chat_id`
...
2. Run `krabkrab logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `krabkrab gateway status`  | Show gateway status           |
| `krabkrab gateway install` | Install/start gateway service |
| `krabkrab gateway stop`    | Stop gateway service          |
| `krabkrab gateway restart` | Restart gateway service       |
| `krabkrab logs --follow`   | Tail gateway logs             |
...
4. Check logs: `krabkrab logs --follow`
...
5. Ensure the gateway is running: `krabkrab gateway status`
6. Check logs: `krabkrab logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.krabkrab/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.krabkrab/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
