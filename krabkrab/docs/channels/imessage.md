...
```
  ...
  <Tab title="Remote Mac over SSH">
    KrabKrab only requires a stdio-compatible `cliPath`, so you can point `cliPath` at a wrapper script that SSHes to a remote Mac and runs `imsg`.
...
```
    ...
    If `remoteHost` is not set, KrabKrab attempts to auto-detect it by parsing the SSH wrapper script.
...
- Full Disk Access is required for the process context running KrabKrab/`imsg` (Messages DB access).
...
<Tabs>
  ...
  <Tab title="Sessions and deterministic replies">
    ...
    If that `chat_id` is explicitly configured under `channels.imessage.groups`, KrabKrab treats it as group traffic (group gating + group session isolation).
...
<AccordionGroup>
  <Accordion title="Dedicated bot macOS user (separate iMessage identity)">
    ...
    4. Create SSH wrapper so KrabKrab can run `imsg` in that user context.
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`krabkrab pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs KrabKrab/`imsg`.
