...
Matrix is an open, decentralized messaging protocol. KrabKrab connects as a Matrix **user**
...
```bash
krabkrab plugins install @krabkrab/matrix
```

Local checkout (when running from a git repo):

```bash
krabkrab plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `krabkrab plugins install @krabkrab/matrix`
   - From a local checkout: `krabkrab plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: KrabKrab calls the same
     login endpoint, stores the access token in `~/.krabkrab/credentials/matrix/credentials.json`,
...
- On first connection, KrabKrab requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  KrabKrab logs a warning.
...
`~/.krabkrab/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `krabkrab pairing list matrix`
  - `krabkrab pairing approve matrix <CODE>`
...
- On startup, KrabKrab resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
