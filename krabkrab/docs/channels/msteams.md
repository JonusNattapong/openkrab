...
KrabKrab will offer the local install path automatically.
...
3. Configure KrabKrab with those credentials.
...
- Talk to KrabKrab via Teams DMs, group chats, or channels.
...
- On startup, KrabKrab resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.krabkrab/krabkrab.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring KrabKrab, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `krabkrab-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `krabkrab plugins install @krabkrab/msteams`
   - From a local checkout: `krabkrab plugins install ./extensions/msteams`
...
KrabKrab handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, KrabKrab only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/krabkrabShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

KrabKrab sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `krabkrab message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.krabkrab/msteams-polls.json`.
