...
```bash
krabkrab plugins install @krabkrab/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
krabkrab plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "KrabKrab" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `krabkrab pairing list nextcloud-talk`
  - `krabkrab pairing approve nextcloud-talk <CODE>`
