...
Tlon is a decentralized messenger built on Urbit. KrabKrab connects to your Urbit ship and can
...
```bash
krabkrab plugins install @krabkrab/tlon
```

Local checkout (when running from a git repo):

```bash
krabkrab plugins install ./extensions/tlon
```
...
By default, KrabKrab blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `krabkrab message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, KrabKrab replies in-thread.
