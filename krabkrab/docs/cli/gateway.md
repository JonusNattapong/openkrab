---
summary: "KrabKrab Gateway CLI (`krabkrab gateway`) â€” run, query, and discover gateways"
read_when:
...
The Gateway is KrabKrabâ€™s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `krabkrab gateway â€¦`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.krabkrab/krabkrab.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
- `--token <token>`: token override (also sets `krabkrab_GATEWAY_TOKEN` for the process).
- `--password <password>`: password override (also sets `krabkrab_GATEWAY_PASSWORD` for the process).
...
`gateway discover` scans for Gateway beacons (`_krabkrab-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `krabkrab.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
