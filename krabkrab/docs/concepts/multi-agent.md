...
- **Session store** (chat history + routing state) under `~/.krabkrab/agents/<agentId>/sessions`.
...
available from `~/.krabkrab/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.krabkrab` (or `krabkrab_STATE_DIR`)
- Workspace: `~/.krabkrab/workspace` (or `~/.krabkrab/workspace-<agentId>`)
- Agent dir: `~/.krabkrab/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.krabkrab/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, KrabKrab runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.krabkrab/workspace` (or `~/.krabkrab/workspace-<profile>` when `krabkrab_PROFILE` is set).
- State defaults to `~/.krabkrab/agents/main/agent`.
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.krabkrab/credentials/whatsapp/personal
          // authDir: "~/.krabkrab/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.krabkrab/credentials/whatsapp/biz
