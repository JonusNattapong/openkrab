...
- `/usage cost` in chats: local cost summary aggregated from KrabKrab session logs.
- CLI: `krabkrab status --usage` prints a full per-provider breakdown.
- CLI: `krabkrab channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
