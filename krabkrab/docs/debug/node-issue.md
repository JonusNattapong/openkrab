...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in KrabKrab
...
Running KrabKrab via Node with `tsx` fails at startup with:

```
[krabkrab] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `krabkrab status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch krabkrab.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node krabkrab.mjs status` works on Node 25.
