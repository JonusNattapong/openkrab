...
read_when:
  - Designing workspace memory (~/.krabkrab/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep KrabKrab integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.krabkrab/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.krabkrab/workspace` as canonical human-readable memory.
...
```
~/.krabkrab/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in KrabKrab**, but keep a separable core library.

### Why integrate into KrabKrab?

- KrabKrab already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `krabkrab memory recall "…" --k 25 --since 30d`
  - `krabkrab memory reflect --since 7d`
...
Pragmatic take for `~/.krabkrab/workspace`:
