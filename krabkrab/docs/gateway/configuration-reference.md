...
description: "Complete field-by-field reference for ~/.krabkrab/krabkrab.json"
---

# Configuration Reference

Every field available in `~/.krabkrab/krabkrab.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional â€” KrabKrab uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `krabkrab doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["krabkrab-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `krabkrab plugins install @krabkrab/mattermost`.
...
KrabKrab spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@krabkrab"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `krabkrab.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, KrabKrab auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, KrabKrab assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.krabkrab/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.krabkrab/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@krabkrab"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:krabkrab`   | All built-in tools (excludes provider plugins)                                           |
...
KrabKrab uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.krabkrab/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `krabkrab_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `krabkrab onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `krabkrab onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `krabkrab onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `krabkrab onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `krabkrab onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `krabkrab onboard --auth-choice minimax-api`.
...
- Loaded from `~/.krabkrab/extensions`, `<workspace>/.krabkrab/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or krabkrab_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `krabkrab_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.krabkrab-dev` + port `19001`), `--profile <name>` (uses `~/.krabkrab-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.krabkrab/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-krabkrab-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `krabkrab_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or krabkrab_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__krabkrab__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__krabkrab__/a2ui/`
...
- Also serves A2UI at `/__krabkrab__/a2ui/`.
...
- Hostname defaults to `krabkrab`. Override with `krabkrab_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.krabkrab/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `krabkrab dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.krabkrab/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${krabkrab_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.krabkrab/credentials/oauth.json`.
...
- Default log file: `/tmp/krabkrab/krabkrab-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `krabkrab doctor --fix` can strip unknown keys).
