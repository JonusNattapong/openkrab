...
read_when:
  - Setting up KrabKrab for the first time
...
KrabKrab reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.krabkrab/krabkrab.json`.

If the file is missing, KrabKrab uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `krabkrab onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    krabkrab onboard       # full setup wizard
    krabkrab configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.krabkrab/krabkrab.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
KrabKrab only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`krabkrab doctor`, `krabkrab logs`, `krabkrab health`, `krabkrab status`)
- Run `krabkrab doctor` to see exact issues
- Run `krabkrab doctor --fix` (or `--yes`) to apply repairs
...
The Gateway watches `~/.krabkrab/krabkrab.json` and applies changes automatically â€” no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `krabkrab config set` for single keys.
    </Warning>
    ...
    krabkrab gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.krabkrab/workspace\" } } }",
      "baseHash": "<hash>",
...
KrabKrab reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.krabkrab/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, KrabKrab runs your login shell and imports only the missing keys:
...
Env var equivalent: `krabkrab_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${krabkrab_GATEWAY_TOKEN}" } },
