...
- During heartbeat runs, KrabKrab treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `krabkrab sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, KrabKrab skips the heartbeat run entirely (no model call).
...
```bash
krabkrab system event --text "Check for urgent follow-ups" --mode now
```
