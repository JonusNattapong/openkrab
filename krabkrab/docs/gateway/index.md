...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `krabkrab_CONFIG_PATH` when set).
...
- Auth is required by default (`gateway.auth.token` / `gateway.auth.password`, or `krabkrab_GATEWAY_TOKEN` / `krabkrab_GATEWAY_PASSWORD`).

### Port and bind precedence

| Setting      | Resolution order                                              |
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` â†’ `krabkrab_GATEWAY_PORT` â†’ `gateway.port` â†’ `18789` |
...
LaunchAgent labels are `ai.krabkrab.gateway` (default) or `ai.krabkrab.<profile>` (named profile). `krabkrab doctor` audits and repairs service config drift.
...
- Unique `gateway.port`
- Unique `krabkrab_CONFIG_PATH`
- Unique `krabkrab_STATE_DIR`
