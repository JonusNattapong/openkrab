---
summary: "Run multiple KrabKrab Gateways on one host (isolation, ports, and profiles)"
read_when:
...
- `krabkrab_CONFIG_PATH` â€” per-instance config file
- `krabkrab_STATE_DIR` â€” per-instance sessions, creds, caches
...
Profiles auto-scope `krabkrab_STATE_DIR` + `krabkrab_CONFIG_PATH` and suffix service names.
...
Base port = `gateway.port` (or `krabkrab_GATEWAY_PORT` / `--port`).
