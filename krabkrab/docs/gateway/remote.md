...
- Use the macOS appâ€™s **Remote over SSH** mode (Settings â†’ General â†’ â€œKrabKrab runsâ€).
...
- `krabkrab health` and `krabkrab status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `krabkrab gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`krabkrab_GATEWAY_PORT`).
