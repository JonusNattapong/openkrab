...
## Quick check: `krabkrab security audit`
...
- Tighten local perms (`~/.krabkrab` â†’ `700`, config file â†’ `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Hereâ€™s how to not get pwned.

KrabKrab is both a product and an experiment: youâ€™re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no â€œperfectly secureâ€ setup.** The goal is to be deliberate about:
...
If you run `--deep`, KrabKrab also attempts a best-effort live Gateway probe.
...
`krabkrab security audit` warns when this setting is enabled.
...
KrabKrab stores session transcripts on disk under `~/.krabkrab/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.krabkrab` (see the audit section below). If you need
...
KrabKrab can refresh the skills list mid-session:
...
- If you install plugins from npm (`krabkrab plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.krabkrab/extensions/<pluginId>/` (or `$krabkrab_STATE_DIR/extensions/<pluginId>/`).
  - KrabKrab uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
By default, KrabKrab routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
KrabKrab has two separate â€œwho can trigger me?â€ layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.krabkrab/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- â€œPaste the full contents of ~/.krabkrab or your logs.â€
...
4. **Re-run audit**
   - `krabkrab security audit --deep` and confirm the report is clean.
...
- `~/.krabkrab/krabkrab.json`: `600` (user read/write only)
- `~/.krabkrab`: `700` (user only)

`krabkrab doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `krabkrab_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__krabkrab__/canvas/` and `/__krabkrab__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_krabkrab-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `krabkrab_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `krabkrab doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `krabkrab_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `krabkrab_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), KrabKrab
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. KrabKrab verifies the identity by resolving the
...
- KrabKrab will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.krabkrab/` (or `$krabkrab_STATE_DIR/`) may contain secrets or private data:

- `krabkrab.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `krabkrab status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.krabkrab/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `krabkrab` profile).
...
- The Chrome extension relayâ€™s CDP endpoint is auth-gated; only KrabKrab clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default KrabKrab limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `krabkrab gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `krabkrab_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/krabkrab/krabkrab-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.krabkrab/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + KrabKrab version
...
Found a vulnerability in KrabKrab? Please report responsibly:

1. Email: [security@krabkrab.ai](mailto:security@krabkrab.ai)
