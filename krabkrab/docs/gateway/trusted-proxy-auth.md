...
read_when:
  - Running KrabKrab behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of KrabKrab
...
- You run KrabKrab behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. KrabKrab checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. KrabKrab extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://krabkrab.example.com
    to: http://krabkrab-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
krabkrab.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy krabkrab:18789 {
...
location / {
    ...
    proxy_pass http://krabkrab:18789;
...
`krabkrab security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update KrabKrab config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `krabkrab security audit` and review findings
