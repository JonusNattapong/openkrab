...
This is handy when you need to toggle obscure settings without editing `krabkrab.json`.
...
- **Global `--dev` (profile):** isolates state under `~/.krabkrab-dev` and
...
If you donâ€™t have a global install yet, run the CLI via `pnpm krabkrab ...`.
...
1. **Profile isolation** (global `--dev`)
   ...
   - `krabkrab_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`krabkrab_SKIP_CHANNELS=1`).
...
## Raw stream logging (KrabKrab)

KrabKrab can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.krabkrab/logs/raw-stream.jsonl
```
