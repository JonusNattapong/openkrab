...
KrabKrab ships three update channels:
...
When you **explicitly** switch channels with `--channel`, KrabKrab also aligns
the install method:

- `dev` ensures a git checkout (default `~/krabkrab`, override with `krabkrab_GIT_DIR`),
...
When you switch channels with `krabkrab update`, KrabKrab also syncs plugin sources:
