...
Deploy KrabKrab on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/krabkrab/krabkrab)
...
repository configures everything needed to run KrabKrab:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: krabkrab_STATE_DIR
        value: /data/.krabkrab
      - key: krabkrab_WORKSPACE_DIR
        value: /data/workspace
      - key: krabkrab_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/krabkrab`.
...
If you use the original KrabKrab repository, Render will not auto-deploy your KrabKrab. To update it, run a manual Blueprint sync from the dashboard.
...
For KrabKrab, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any KrabKrab host.
