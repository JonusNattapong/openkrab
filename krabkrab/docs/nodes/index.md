...
macOS can also run in **node mode**: the menubar app connects to the Gatewayâ€™s WS server and exposes its local canvas/camera commands as a node (so `krabkrab nodes â€¦` works against this Mac).
...
- `node.pair.*` (CLI: `krabkrab nodes pending/approve/reject`) is a separate gateway-owned
...
- **Approvals**: enforced on the node host via `~/.krabkrab/exec-approvals.json`.
...
- The token is `gateway.auth.token` from the gateway config (`~/.krabkrab/krabkrab.json` on the gateway host).
- `krabkrab node run` reads `krabkrab_GATEWAY_TOKEN` for auth.
...
- `--display-name` on `krabkrab node run` / `krabkrab node install` (persists in `~/.krabkrab/node.json` on the node).
- `krabkrab nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
Approvals live on the node host at `~/.krabkrab/exec-approvals.json`.
...
krabkrab nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
...
- On headless node host, `system.run` is gated by exec approvals (`~/.krabkrab/exec-approvals.json`).
...
KrabKrab can run a **headless node host** (no UI) that connects to the Gateway
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.krabkrab/node.json`.
- Exec approvals are enforced locally via `~/.krabkrab/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host prefers the companion app exec host when reachable and falls
  back to local execution if the app is unavailable. Set `krabkrab_NODE_EXEC_HOST=app` to require
  the app, or `krabkrab_NODE_EXEC_FALLBACK=0` to disable fallback.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `krabkrab nodes â€¦` works against this Mac).
