...
service via the external `krabkrab` CLI (no embedded runtime). This gives you
...
- The app installs a perâ€‘user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`krabkrab_PROFILE`; legacy `com.krabkrab.*` is supported).
...
- Writes `~/.krabkrab/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.krabkrab/disable-launchagent`,
