---
summary: "KrabKrab macOS companion app (menu bar + gateway broker)"
read_when:
...
# KrabKrab macOS Companion (menu bar + gateway broker)

The macOS app is the **menuâ€‘bar companion** for KrabKrab. It owns permissions,
...
- Installs the global CLI (`krabkrab`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `krabkrab gateway install`.
...
(or `bot.molt.<profile>` when using `--profile`/`krabkrab_PROFILE`; legacy `com.krabkrab.*` still unloads).
...
The app registers the `krabkrab://` URL scheme for local actions.
...
1. Install and launch **KrabKrab.app**.
...
Tip: compare against `krabkrab gateway discover --json` to see whether the
