---
summary: "KrabKrab on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up KrabKrab on a Raspberry Pi
  - Running KrabKrab on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# KrabKrab on Raspberry Pi

## Goal

Run a persistent, always-on KrabKrab Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most KrabKrab features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled krabkrab
...
```bash
# Check logs
journalctl -u krabkrab --no-pager -n 100

# Common fix: rebuild
cd ~/krabkrab  # if using hackable install
npm run build
sudo systemctl restart krabkrab
```
