...
read_when:
  - You want to place an outbound voice call from KrabKrab
...
Voice calls for KrabKrab via a plugin. Supports outbound notifications and
...
- Use `krabkrab voicecall ...` or the `voice_call` tool
...
```bash
krabkrab plugins install ./extensions/voice-call
...
```bash
krabkrab voicecall call --to "+15555550123" --message "Hello from KrabKrab"
krabkrab voicecall continue --call-id <id> --message "Any questions?"
krabkrab voicecall speak --call-id <id> --message "One moment"
krabkrab voicecall end --call-id <id>
krabkrab voicecall status --call-id <id>
krabkrab voicecall tail
