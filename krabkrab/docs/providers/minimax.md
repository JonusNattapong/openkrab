---
summary: "Use MiniMax M2.1 in KrabKrab"
read_when:
  - You want MiniMax models in KrabKrab
...
```bash
krabkrab plugins enable minimax-portal-auth  # skip if already loaded.
krabkrab gateway restart  # restart if gateway is already running
krabkrab onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/krabkrab/krabkrab/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `krabkrab.json`:
...
- Use `krabkrab models list` and `krabkrab models set minimax/MiniMax-M2.1` to switch.
...
- Running `krabkrab configure` and selecting **MiniMax M2.1**, or
