---
summary: "Use Venice AI privacy-focused models in KrabKrab"
read_when:
  - You want privacy-focused inference in KrabKrab
...
```bash
krabkrab onboard --auth-choice venice-api-key
```
...
```bash
krabkrab chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, KrabKrab shows all available Venice models. Pick based on your needs:
...
```bash
krabkrab models set venice/claude-opus-45
krabkrab models set venice/llama-3.3-70b
```

List all available models:

```bash
krabkrab models list | grep venice
```

## Configure via `krabkrab configure`

1. Run `krabkrab configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
KrabKrab automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
krabkrab chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
krabkrab chat --model venice/claude-opus-45

# Use uncensored model
krabkrab chat --model venice/venice-uncensored

# Use vision model with image
krabkrab chat --model venice/qwen3-vl-235b-a22b

# Use coding model
krabkrab chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `krabkrab models list` to see currently available models. Some models may be temporarily offline.
