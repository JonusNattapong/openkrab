...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.krabkrab/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.krabkrab/node.json` (node id + pairing token).
...
Path: `~/.krabkrab/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.krabkrab/exec-approvals.sock",
...
- Unix socket at `~/.krabkrab/exec-approvals.sock` (0600).
