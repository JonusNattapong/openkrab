...
- Publish as `krabkrab/plugin-sdk` (or export from core under `krabkrab/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `krabkrabPluginApi.runtime` so plugins never import `src/**`.
...
- Introduce `krabkrab/plugin-sdk`.
- Add `api.runtime` to `krabkrabPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `krabkrabRuntime: ">=2026.2.0"`).
