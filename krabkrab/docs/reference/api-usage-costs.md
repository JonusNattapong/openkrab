...
KrabKrab features that can generate provider usage or paid API calls.
...
- `krabkrab status --usage` and `krabkrab channels list` show provider **usage windows**
...
KrabKrab can pick up credentials from:
...
- `krabkrab status --usage`
- `krabkrab models status --json`
...
`krabkrab models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
