...
KrabKrab integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- KrabKrab owns lifecycle when `channels.signal.autoStart=true`.
...
- KrabKrab spawns `imsg rpc` as a child process (legacy iMessage integration).
