...
read_when:
  ...
  - Debugging why krabkrab is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (KrabKrab-managed)"
---

# Browser (krabkrab-managed)

KrabKrab can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `krabkrab` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `krabkrab` for the isolated managed browser.

## What you get

- A separate browser profile named **krabkrab** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`krabkrab`, `work`, `remote`, ...).
...
## Profiles: `krabkrab` vs `chrome`

- `krabkrab`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the KrabKrab
  extension to be attached to a tab).

Set `browser.defaultProfile: "krabkrab"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.krabkrab/krabkrab.json`.
...
- If you override the Gateway port (`gateway.port` or `krabkrab_GATEWAY_PORT`),
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "krabkrab"` for the managed browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome â†’ Brave â†’ Edge â†’ Chromium â†’ Chrome Canary.
- Local `krabkrab` profiles auto-assign `cdpPort`/`cdpUrl` â€” set those only for remote CDP.

## Use Brave (or another Chromium-based browser)

If your **system default** browser is Chromium-based (Chrome/Brave/Edge/etc),
KrabKrab uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, KrabKrab will not launch a local browser.
...
KrabKrab preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, KrabKrab can
...
CDP endpoints over HTTPS. You can point a KrabKrab browser profile at a
...
- If browser control is enabled and no auth is configured, KrabKrab auto-generates `gateway.auth.token` on startup and persists it to config.
...
KrabKrab supports multiple named profiles (routing configs). Profiles can be:

- **krabkrab-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **extension relay**: your existing Chrome tab(s) via the local relay + Chrome extension

Defaults:

- The `krabkrab` profile is auto-created if missing.
...
KrabKrab can also drive **your existing Chrome tabs** (no separate â€œkrabkrabâ€ Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **KrabKrab Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
- â€œLoad unpackedâ€ â†’ select the directory printed by `krabkrab browser extension path`
- Pin the extension, then click it on the tab you want to control (badge shows `ON`).

2. Use it:

- CLI: `krabkrab browser --browser-profile chrome tabs`
...
When launching locally, KrabKrab picks the first available:
...
- `x-krabkrab-password: <gateway password>` or HTTP Basic auth with that password

### Playwright requirement

Some features (navigate/act/AI snapshot/role snapshot, element screenshots, PDF) require
Playwright. If Playwright isnâ€™t installed, those endpoints return a clear 501
error. ARIA snapshots and basic screenshots still work for krabkrab-managed Chrome.
For the Chrome extension relay driver, ARIA snapshots and screenshots require Playwright.

If you see `Playwright is not available in this gateway build`, install the full
Playwright package (not `playwright-core`) and restart the gateway, or reinstall
KrabKrab with browser support.
...
`krabkrab_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `krabkrab browser download e12 report.pdf`
- `krabkrab browser waitfordownload report.pdf`
- `krabkrab browser upload /tmp/krabkrab/uploads/file.pdf`
...
- Download and trace output paths are constrained to KrabKrab temp roots:
  - traces: `/tmp/krabkrab` (fallback: `${os.tmpdir()}/krabkrab`)
  - downloads: `/tmp/krabkrab/downloads` (fallback: `${os.tmpdir()}/krabkrab/downloads`)
- Upload paths are constrained to an KrabKrab temp uploads root:
  - uploads: `/tmp/krabkrab/uploads` (fallback: `${os.tmpdir()}/krabkrab/uploads`)
- `upload` can also set file inputs directly via `--input-ref` or `--element`.
- `snapshot`:
  - `--format ai` (default when Playwright is installed): returns an AI snapshot with numeric refs (`aria-ref="<n>"`).
  - `--format aria`: returns the accessibility tree (no refs; inspection only).
  - `--efficient` (or `--mode efficient`): compact role snapshot preset (interactive + compact + depth + lower maxChars).
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-krabkrab-managed-browser)).
...
KrabKrab supports two â€œsnapshotâ€ styles:

- **AI snapshot (numeric refs)**: `krabkrab browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `krabkrab browser click 12`, `krabkrab browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwrightâ€™s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `krabkrab browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `krabkrab browser click e12`, `krabkrab browser highlight e12`.
...
3. If it still fails: `krabkrab browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `krabkrab browser trace start`
   - reproduce the issue
   - `krabkrab browser trace stop` (prints `TRACE:<path>`)
...
- The krabkrab browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `krabkrab browser evaluate` and `wait --fn`
...
- `browser` accepts:
  - `profile` to choose a named browser profile (krabkrab, chrome, or remote CDP).
