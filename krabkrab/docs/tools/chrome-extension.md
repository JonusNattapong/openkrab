---
summary: "Chrome extension: let KrabKrab drive your existing Chrome tab"
read_when:
...
The KrabKrab Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate krabkrab-managed Chrome profile.
...
KrabKrab then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the KrabKrab release (npm package) as static files. There is no separate “build” step.

After upgrading KrabKrab:

- Re-run `krabkrab browser extension install` to refresh the installed files under your KrabKrab state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

KrabKrab ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want KrabKrab to control.
...
- `ON`: attached; KrabKrab can drive that tab.
...
Debugging: `krabkrab sandbox explain`
...
`krabkrab browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `krabkrab browser extension install` first to copy the extension to a stable location under your KrabKrab state directory.
...
- **This is not isolated** like the dedicated krabkrab-managed profile.
