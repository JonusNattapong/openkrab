...
- **gateway host** → `krabkrab` process on the gateway machine
...
`~/.krabkrab/exec-approvals.json` directly.

CLI: `krabkrab approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
