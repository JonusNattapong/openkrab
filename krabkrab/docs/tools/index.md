---
summary: "Agent tool surface for KrabKrab (browser, canvas, nodes, message, cron) replacing legacy `krabkrab-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `krabkrab-*` skills
title: "Tools"
---

# Tools (KrabKrab)

KrabKrab exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `krabkrab-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `krabkrab.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, KrabKrab logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:krabkrab`: all built-in KrabKrab tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`krabkrab node run`).
...
KrabKrab tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `krabkrab configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated KrabKrab-managed browser.
...
- Quick smoke: `krabkrab nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `krabkrab gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, KrabKrab clamps `tools.sessions.visibility` to `tree`.
