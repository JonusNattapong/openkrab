...
For debugging â€œwhy is this blocked?â€, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `krabkrab sandbox explain`.
...
Legacy `agent.*` configs are migrated by `krabkrab doctor`; prefer `agents.defaults` + `agents.list` going forward.
