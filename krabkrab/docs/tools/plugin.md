---
summary: "KrabKrab plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends KrabKrab with extra
features (commands, tools, and Gateway RPC).

Most of the time, youâ€™ll use plugins when you want a feature thatâ€™s not built
into core KrabKrab yet (or you want to keep optional features out of your main
install).
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@krabkrab/msteams` if you use Teams.
...
KrabKrab plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
4. Bundled extensions (shipped with KrabKrab, **disabled by default**)

- `<krabkrab>/extensions/*`

Bundled plugins must be enabled explicitly via `plugins.entries.<id>.enabled`
or `krabkrab plugins enable <id>`. Installed plugins are enabled by default,
but can be disabled the same way.

Each plugin must include a `krabkrab.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `krabkrab.extensions`:
...
Security note: `krabkrab plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `krabkrab.channel` and
install hints via `krabkrab.install`. This keeps the core catalog data-free.
...
KrabKrab can also merge **external channel catalogs** (for example, an MPM
...
Or point `krabkrab_PLUGIN_CATALOG_PATHS` (or `krabkrab_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "krabkrab": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, KrabKrab uses it but warns when it doesnâ€™t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `krabkrab.plugin.json` (`configSchema`).
...
KrabKrab augments `uiHints` at runtime based on discovered plugins:
...
Plugins may also register their own topâ€‘level commands (example: `krabkrab voicecall`).
...
- Plugin-managed hooks show up in `krabkrab hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `krabkrab hooks`; enable/disable the plugin instead.

## Provider plugins (model auth)

Plugins can register **model provider auth** flows so users can run OAuth or
API-key setup inside KrabKrab (no external scripts needed).
...
- `config`: The current KrabKrab config
...
- Main package: `krabkrab` (this repo)
- Plugins: separate npm packages under `@krabkrab/*` (example: `@krabkrab/voice-call`)

Publishing contract:

- Plugin `package.json` must include `krabkrab.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `krabkrab plugins install <npm-spec>` uses `npm pack`, extracts into `~/.krabkrab/extensions/<id>/`, and enables it in config.
...
- CLI: `krabkrab voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `krabkrab.extensions` points at the built entrypoint (`dist/index.js`).
