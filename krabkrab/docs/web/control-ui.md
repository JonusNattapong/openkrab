...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/krabkrab`)
...
If the page fails to load, start the Gateway first: `krabkrab gateway`.
...
you revoke it with `krabkrab devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.krabkrab/krabkrab.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. KrabKrab
...
KrabKrab **blocks** Control UI connections without device identity.
