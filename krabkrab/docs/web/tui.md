...
```bash
krabkrab tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `krabkrab tui --deliver`
...
- Check the Gateway logs: `krabkrab logs --follow`.
- Confirm the agent can run: `krabkrab status` and `krabkrab models status`.
...
- No agents in picker: check `krabkrab agents list` and your routing config.
