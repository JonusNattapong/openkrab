...
/// Minimal registry stub for channels.
pub struct Registry {
...
fn chat_meta_map() -> HashMap<&'static str, ChatChannelMeta> {
    ...
    m.insert(
        ...
        ChatChannelMeta {
            ...
            blurb: Some("simplest way to get started — register a bot with @BotFather and get going.".into()),
...
pub fn normalize_chat_channel_id(raw: Option<&str>) -> Option<String> {
    ...
    .into_iter()
    .collect();
    let resolved = aliases.get(normalized.as_str()).map(|s| s.to_string()).unwrap_or(normalized.clone());
    ...
    F: Fn(&str, Option<&str>) -> String,
{
    ...
    let extras = meta.selection_extras.as_ref().map(|v| v.join(" ")).unwrap_or_default();
    format!("{} — {} {}{}", meta.label, meta.blurb.clone().unwrap_or_default(), if docs_prefix.is_empty() { "".to_string() } else { format!("{} ", docs_prefix) }, docs)
}
