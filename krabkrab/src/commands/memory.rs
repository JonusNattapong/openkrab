...
pub async fn memory_sync_command(workspace_dir: &str, db_path: Option<&str>, watch: bool) -> Result<String> {
    ...
    if watch {
        ...
        Ok(format!("Successfully synced and now watching workspace: {}", workspace_dir))
    } else {
...
pub async fn memory_search_command(query: &str, db_path: Option<&str>) -> Result<String> {
    ...
    for (i, res) in results.iter().enumerate() {
        out.push_str(&format!(
            "{}. [{:.4}] {} (L{}-L{})\n   {}\n\n",
            i + 1, res.score, res.path, res.start_line, res.end_line,
