---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenKrab deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenKrab cron scheduling for periodic checks, or version status checks on a machine running OpenKrab (laptop, workstation, Pi, VPS).
---

# OpenKrab Host Hardening

## Overview

Assess and harden the host running OpenKrab, then align it to a user-defined risk tolerance without breaking access. Use OpenKrab security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenKrab changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenKrab gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenKrab can access sensitive data.
...
### 2) Run OpenKrab security audits (read-only)

As part of the default read-only checks, run `OpenKrab security audit --deep`. Only offer alternatives if the user requests them:

1. `OpenKrab security audit` (faster, non-probing)
2. `OpenKrab security audit --json` (structured output)

Offer to apply OpenKrab safe defaults (numbered):

1. `OpenKrab security audit --fix`

Be explicit that `--fix` only tightens OpenKrab defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenKrab version/update status (read-only)

As part of the default read-only checks, run `OpenKrab update status`.
...
- Credential hygiene notes (location of OpenKrab creds, prefer disk encryption)
...
- OpenKrab security audit (re-run)
...
After OpenKrab install or first hardening pass, run at least one baseline audit and version check:

- `OpenKrab security audit`
- `OpenKrab security audit --deep`
- `OpenKrab update status`

Ongoing monitoring is recommended. Use the OpenKrab cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `OpenKrab cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `OpenKrab update status`
...
Before creating, `OpenKrab cron list` and match on exact `name`. If found, `OpenKrab cron edit <id> ...`.
If not found, `OpenKrab cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `OpenKrab update status` (preferred for source checkouts and channels)
2. `npm view OpenKrab version` (published npm version)

## OpenKrab command accuracy

Use only supported commands and flags:

- `OpenKrab security audit [--deep] [--fix] [--json]`
- `OpenKrab status` / `OpenKrab status --deep`
- `OpenKrab health --json`
- `OpenKrab update status`
- `OpenKrab cron add|list|runs|run`

Do not invent CLI flags or imply OpenKrab enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenKrab compaction:
