...
pub async fn health_command(cfg: &AppConfig, timeout_ms: u64) -> HealthResult {
    ...
    let gateway_check = HealthCheck {
        ...
        status: if gateway_status { CheckStatus::Pass } else { CheckStatus::Fail },
    ...
    let memory_check = HealthCheck {
        ...
        status: if cfg.memory.enabled { CheckStatus::Pass } else { CheckStatus::Warn },
    ...
    let auth_check = HealthCheck {
        name: "auth".to_string(),
        status: if auth_count > 0 { CheckStatus::Pass } else { CheckStatus::Warn },
        message: format!("{} auth profile(s) configured", auth_count),
        duration_ms: 0,
    };
    checks.push(auth_check);
    
    let healthy = checks.iter().all(|c| !matches!(c.status, CheckStatus::Fail));
