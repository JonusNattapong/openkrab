...
mod tests {
    ...
    #[test]
    fn resolve_effect_id_known_effects() {
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some("com.apple.messages.effect.CKHeartEffect".to_string())
        );
        assert_eq!(
            resolve_effect_id(Some("confetti")),
            Some("com.apple.messages.effect.CKConfettiEffect".to_string())
        );
    }

    #[test]
    fn resolve_effect_id_unknown() {
        assert_eq!(resolve_effect_id(Some("unknown_effect")), None);
        assert_eq!(resolve_effect_id(None), None);
    }

    #[test]
    fn bluebubbles_account_new() {
        let acc = BlueBubblesAccount::new("http://localhost:8080", "secret");
        assert_eq!(acc.server_url, "http://localhost:8080");
