...
describe("canvas host state dir defaults", () => {
  ...
  it("uses krabkrab_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "krabkrab-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("krabkrab Canvas");
    } finally {
      await handler.close();
