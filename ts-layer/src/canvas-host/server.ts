...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.krabkrabCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.krabkrabCanvasA2UIAction &&
        typeof window.krabkrabCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.krabkrabSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("krabkrab:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android krabkrab node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.krabkrabSendUserAction === "function"
        ? window.krabkrabSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.krabkrab_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.krabkrab_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>krabkrab Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
