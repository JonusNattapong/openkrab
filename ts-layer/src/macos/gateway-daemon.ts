...
declare const __krabkrab_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __krabkrab_VERSION__ === "string" && __krabkrab_VERSION__) ||
  process.env.krabkrab_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `krabkrab --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.krabkrab_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.krabkrab_GATEWAY_BIND ??
  ...
  if (token) {
    process.env.krabkrab_GATEWAY_TOKEN = token;
  }
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (krabkrab_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
...
void main().catch((err) => {
  console.error(
    "[krabkrab] Gateway daemon failed:",
