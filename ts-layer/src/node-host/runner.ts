...
import { ensurekrabkrabCliOnPath } from "../infra/path-env.js";
...
function ensureNodePathEnv(): string {
  ensurekrabkrabCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function runNodeHost(opts: NodeHostRunOptions): Promise<void> {
  ...
  const token =
    process.env.krabkrab_GATEWAY_TOKEN?.trim() ||
    (isRemoteMode ? cfg.gateway?.remote?.token : cfg.gateway?.auth?.token);
  const password =
    process.env.krabkrab_GATEWAY_PASSWORD?.trim() ||
