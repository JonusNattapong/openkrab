...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["krabkrab_OAUTH_DIR"]);
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "krabkrab-oauth-"));
    process.env.krabkrab_OAUTH_DIR = tempOauthDir;
  });
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "krabkrab-wa-auth-"));
    try {
