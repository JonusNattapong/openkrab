...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("krabkrab channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    runtime.error(
      `WhatsApp Web connection closed (status ${statusCode}). Retry ${reconnectAttempts}/${reconnectPolicy.maxAttempts || "âˆž"} in ${formatDurationPrecise(delay)}â€¦ (${errorStr})`,
    );
    await closeListener();
    try {
