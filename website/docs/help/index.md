...
```bash\nOpenKrab doctor          # Check configuration\nOpenKrab status          # Check gateway status\nOpenKrab logs --follow   # View logs
```

### Can't connect to channel

1. Check channel configuration: `openkrab channels status`
2. Verify API tokens are set correctly
3. Check gateway logs for errors

### AI not responding

1. Verify provider API key: `openkrab config get providers.openai.api_key`
2. Check model status: `openkrab models status`
3. Test with simple query: `openkrab ask "hello"`
