...
1. Install Lume
2. `lume create OpenKrab --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run OpenKrab --no-display`
...
```bash
lume create OpenKrab --os macos --ipsw latest
```
...
```bash
ssh youruser@192.168.64.X "OpenKrab status"
```
...
```bash
lume stop OpenKrab
lume clone OpenKrab OpenKrab-golden
```

Reset anytime:

```bash
lume stop OpenKrab && lume delete OpenKrab
lume clone OpenKrab-golden OpenKrab
lume run OpenKrab --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get OpenKrab` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                    |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `OpenKrab channels login` |
