...
Run the OpenKrab gateway in a **rootless** Podman container. Uses the same image as Docker (build from the repo [Dockerfile](https://github.com/OpenKrab/OpenKrab/blob/main/Dockerfile)).
...
This also creates a minimal `~OpenKrab/.OpenKrab/OpenKrab.json` (sets `gateway.mode="local"`) so the gateway can start without running the wizard.
...
```bash
./scripts/run-OpenKrab-podman.sh launch
```

**3. Onboarding wizard** (e.g. to add channels or providers):

```bash
./scripts/run-OpenKrab-podman.sh launch setup
```

Then open `http://127.0.0.1:18789/` and use the token from `~OpenKrab/.OpenKrab/.env` (or the value printed by setup).
...
- **Status:** `sudo systemctl --machine OpenKrab@ --user status OpenKrab.service`
- **Logs:** `sudo journalctl --machine OpenKrab@ --user -u OpenKrab.service -f`

The quadlet file lives at `~OpenKrab/.config/containers/systemd/OpenKrab.container`. To change ports or env, edit that file (or the `.env` it sources), then `sudo systemctl --machine OpenKrab@ --user daemon-reload` and restart the service. On boot, the service starts automatically if lingering is enabled for OpenKrab (setup does this when loginctl is available).

To add quadlet **after** an initial setup that did not use it, re-run: `./setup-podman.sh --quadlet`.

## The OpenKrab user (non-login)
...
- **Rootless Podman:** The user must have a **subuid** and **subgid** range. Many distros assign these automatically when the user is created. If setup prints a warning, add lines to `/etc/subuid` and `/etc/subgid`:
  ...
  ```text
  OpenKrab:100000:65536
  ```

  Then start the gateway as that user (e.g. from cron or systemd):

  ```bash
  sudo -u OpenKrab /home/OpenKrab/run-OpenKrab-podman.sh
  sudo -u OpenKrab /home/OpenKrab/run-OpenKrab-podman.sh setup
  ```

- **Config:** Only `OpenKrab` and root can access `/home/OpenKrab/.OpenKrab`. To edit config: use the Control UI once the gateway is running, or `sudo -u OpenKrab $EDITOR /home/OpenKrab/.OpenKrab/OpenKrab.json`.
