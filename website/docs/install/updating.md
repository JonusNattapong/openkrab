...
detects existing installs, upgrades in place, and runs `OpenKrab doctor` when
needed.
...
1. Runs the same source-update flow as `OpenKrab update` (git checkout only).
...
- Windows (WSL2): `systemctl --user restart OpenKrab-gateway[-<profile>].service`
  - `launchctl`/`systemctl` only work if the service is installed; otherwise run `OpenKrab gateway install`.
...
Tip: to see the current published version, run `npm view OpenKrab version`.
