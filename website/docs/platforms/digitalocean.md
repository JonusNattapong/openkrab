...
# Check service
systemctl --user status OpenKrab-gateway.service

# View logs
journalctl --user -u OpenKrab-gateway.service -f
```
...
```bash
OpenKrab config set gateway.bind tailnet
OpenKrab gateway restart
```
...
```bash
OpenKrab channels login whatsapp
# Scan QR code
```
...
- `~/.OpenKrab/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u OpenKrab --no-pager -n 50
```
