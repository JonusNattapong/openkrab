...
- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `OpenKrab gateway install`.
...
The app registers the `OpenKrab://` URL scheme for local actions.
...
```bash
cd apps/macos
swift run OpenKrab-mac connect --json
