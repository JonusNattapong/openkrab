...
```bash
OpenKrab plugins enable minimax-portal-auth  # skip if already loaded.
OpenKrab gateway restart  # restart if gateway is already running
OpenKrab onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/OpenKrab/OpenKrab/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `OpenKrab.json`:
...
- Use `OpenKrab models list` and `OpenKrab models set minimax/MiniMax-M2.1` to switch.
...
- Running `OpenKrab configure` and selecting **MiniMax M2.1**, or
