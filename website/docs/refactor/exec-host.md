...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.OpenKrab/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.OpenKrab/node.json` (node id + pairing token).
...
Path: `~/.OpenKrab/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.OpenKrab/exec-approvals.sock",
...
- Unix socket at `~/.OpenKrab/exec-approvals.sock` (0600).
