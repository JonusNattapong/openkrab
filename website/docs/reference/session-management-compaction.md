...
- Transcripts: `~/.OpenKrab/agents/<agentId>/sessions/<sessionId>.jsonl`
...
- `OpenKrab status` (CLI)
- `OpenKrab sessions` / `sessions --json`
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `OpenKrab status`.
