...
flowchart TB
    ...
    B -- linked via QR --> C["<b>Your Mac (OpenKrab)<br></b><br>Pi agent"]
```
...
3. Put a minimal config in `~/.OpenKrab/OpenKrab.json`:
...
When onboarding finishes, we auto-open the dashboard and print a clean (non-tokenized) link. If it prompts for auth, paste the token from `gateway.auth.token` into Control UI settings. To reopen later: `OpenKrab dashboard`.

## Give the agent a workspace (AGENTS)

OpenKrab reads operating instructions and “memory” from its workspace directory.

By default, OpenKrab uses `~/.OpenKrab/workspace` as the agent workspace, and will create it (plus starter `AGENTS.md`, `SOUL.md`, `TOOLS.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md`) automatically on setup/first agent run. `BOOTSTRAP.md` is only created when the workspace is brand new (it should not come back after you delete it). `MEMORY.md` is optional (not auto-created); when present, it is loaded for normal sessions. Subagent sessions only inject `AGENTS.md` and `TOOLS.md`.
...
- Session files: `~/.OpenKrab/agents/<agentId>/sessions/{{SessionId}}.jsonl`
- Session metadata (token usage, last route, etc): `~/.OpenKrab/agents/<agentId>/sessions/sessions.json` (legacy: `~/.OpenKrab/sessions/sessions.json`)
...
```bash
OpenKrab status          # local status (creds, sessions, queued events)
OpenKrab status --all    # full diagnosis (read-only, pasteable)
OpenKrab status --deep   # adds gateway health probes (Telegram + Discord)
OpenKrab health --json   # gateway health snapshot (WS)
```

Logs live under `/tmp/OpenKrab/` (default: `OpenKrab-YYYY-MM-DD.log`).
