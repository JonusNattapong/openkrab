...
This page is the full reference for `OpenKrab onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.OpenKrab/OpenKrab.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `OpenKrab doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `OpenKrab pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `OpenKrab health`.
    - `OpenKrab status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.OpenKrab/.env` so launchd can read it.
...
- OAuth credentials: `~/.OpenKrab/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.OpenKrab/agents/<agentId>/agent/auth-profiles.json`
