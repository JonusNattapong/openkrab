---
summary: "Direct `OpenKrab agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `OpenKrab agent` (direct agent runs)

`OpenKrab agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `OpenKrab message --target`).
...
```bash
OpenKrab agent --to +15555550123 --message "status update"
OpenKrab agent --agent ops --message "Summarize logs"
OpenKrab agent --session-id 1234 --message "Summarize inbox" --thinking medium
OpenKrab agent --to +15555550123 --message "Trace logs" --verbose on --json
OpenKrab agent --to +15555550123 --message "Summon reply" --deliver
