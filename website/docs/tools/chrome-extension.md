...
The OpenKrab Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate OpenKrab-managed Chrome profile.
...
The CLI intentionally does **not** print a `node_modules` path. Always run `OpenKrab browser extension install` first to copy the extension to a stable location under your OpenKrab state directory.
