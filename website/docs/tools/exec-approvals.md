...
- **gateway host** → `OpenKrab` process on the gateway machine
...
`~/.OpenKrab/exec-approvals.json` directly.

CLI: `OpenKrab approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
