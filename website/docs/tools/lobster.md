...
gog.gmail.search --query 'newer_than:1d' \
  | OpenKrab.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
OpenKrab.invoke --tool llm-task --action json --args-json '{
...
Install the Lobster CLI on the **same host** that runs the OpenKrab Gateway (see the [Lobster repo](https://github.com/OpenKrab/lobster)), and ensure `lobster` is on `PATH`.
...
→ OpenKrab calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ OpenKrab calls gmail.send
