...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@OpenKrab/msteams` if you use Teams.
...
or `OpenKrab plugins enable <id>`. Installed plugins are enabled by default,
...
Each plugin must include a `OpenKrab.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `OpenKrab.extensions`:
...
Security guardrail: every `OpenKrab.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `OpenKrab plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `OpenKrab.channel` and
install hints via `OpenKrab.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@OpenKrab/nextcloud-talk",
  "OpenKrab": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@OpenKrab/nextcloud-talk",
...
Or point `OPENKRAB_PLUGIN_CATALOG_PATHS` (or `OPENKRAB_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "OpenKrab": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, OpenKrab uses it but warns when it doesnâ€™t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `OpenKrab.plugin.json` (`configSchema`).
...
OpenKrab plugins install <path>                 # copy a local file/dir into ~/.OpenKrab/extensions/<id>
OpenKrab plugins install ./extensions/voice-call # relative path ok
OpenKrab plugins install ./plugin.tgz           # install from a local tarball
OpenKrab plugins install ./plugin.zip           # install from a local zip
OpenKrab plugins install -l ./extensions/voice-call # link (no copy) for dev
OpenKrab plugins install @OpenKrab/voice-call # install from npm
OpenKrab plugins install @OpenKrab/voice-call --pin # store exact resolved name@version
...
Plugins may also register their own topâ€‘level commands (example: `OpenKrab voicecall`).
...
- Plugin-managed hooks show up in `OpenKrab hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `OpenKrab hooks`; enable/disable the plugin instead.
...
- Main package: `OpenKrab` (this repo)
- Plugins: separate npm packages under `@OpenKrab/*` (example: `@OpenKrab/voice-call`)

Publishing contract:

- Plugin `package.json` must include `OpenKrab.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `OpenKrab plugins install <npm-spec>` uses `npm pack`, extracts into `~/.OpenKrab/extensions/<id>/`, and enables it in config.
...
- Separately published plugins should run their own CI (lint/build/test) and validate `OpenKrab.extensions` points at the built entrypoint (`dist/index.js`).
