...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/OpenKrab`)
...
If the page fails to load, start the Gateway first: `OpenKrab gateway`.
...
you revoke it with `OpenKrab devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.OpenKrab/OpenKrab.json` (`config.get`, `config.set`)
