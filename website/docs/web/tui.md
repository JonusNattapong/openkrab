...
```bash
OpenKrab tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `OpenKrab tui --deliver`
...
- Check the Gateway logs: `OpenKrab logs --follow`.
- Confirm the agent can run: `OpenKrab status` and `OpenKrab models status`.
...
- No agents in picker: check `OpenKrab agents list` and your routing config.
