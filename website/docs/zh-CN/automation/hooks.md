...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 OpenKrab 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `OpenKrab webhooks` 获取 Gmail 助手命令。
...
- **💾 session-memory**：当你发出 `/new` 时将会话上下文保存到智能体工作区（默认 `~/.OpenKrab/workspace/memory/`）
- **📝 command-logger**：将所有命令事件记录到 `~/.OpenKrab/logs/commands.log`
...
在新手引导期间（`OpenKrab onboard`），你将被提示启用推荐的 hooks。向导会自动发现符合条件的 hooks 并呈现供选择。
...
2. **托管 hooks**：`~/.OpenKrab/hooks/`（用户安装，跨工作区共享）
3. **捆绑 hooks**：`<OpenKrab>/dist/hooks/bundled/`（随 OpenKrab 附带）
...
Hook 包是标准的 npm 包，通过 `package.json` 中的 `OpenKrab.hooks` 导出一个或多个 hooks。使用以下命令安装：
...
Hook 包可以附带依赖；它们将安装在 `~/.OpenKrab/hooks/<id>` 下。
...
- **托管 hooks**（`~/.OpenKrab/hooks/`）：跨工作区共享
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认为 `~/.OpenKrab/workspace`）
...
- [捆绑 Hooks README](https://github.com/OpenKrab/OpenKrab/tree/main/src/hooks/bundled)
