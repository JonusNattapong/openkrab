...
3. 运行 `OpenKrab onboard` 并选择 BlueBubbles，或手动配置：
   ```json5
   {
     channels: {
       bluebubbles: {
...
- 配对码在一小时后过期；使用 `OpenKrab pairing list bluebubbles` 和 `OpenKrab pairing approve bluebubbles <code>`。
...
- 查看状态/健康信息：`OpenKrab status --all` 或 `OpenKrab status --deep`。
