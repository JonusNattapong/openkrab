...
2. 创建 macOS 用户（例如：`OpenKrabhome`）并登录。
...
│ - OpenKrab gateway           │          SCP (attachments)        │ - Messages signed in     │
...
示例包装脚本（`~/.OpenKrab/scripts/imsg-ssh`）：
...
多账户支持：使用 `channels.imessage.accounts` 配置每个账户及可选的 `name`。参见 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) 了解共享模式。不要提交 `~/.OpenKrab/OpenKrab.json`（它通常包含令牌）。
