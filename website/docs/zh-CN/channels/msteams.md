...
5. 在 `~/.OpenKrab/OpenKrab.json`（或环境变量）中配置 `msteams` 并启动 Gateway 网关。
...
2. 填写**基本信息**选项卡：
   ...
   | 字段               | 值                                                  |
   | ------------------ | --------------------------------------------------- |
   | **Bot handle**     | 你的机器人名称，例如 `OpenKrab-msteams`（必须唯一） |
...
1. **安装 Microsoft Teams 插件**
   - 从 npm：`OpenKrab plugins install @OpenKrab/msteams`
   - 从本地检出：`OpenKrab plugins install ./extensions/msteams`
...
- 投票由 Gateway 网关记录在 `~/.OpenKrab/msteams-polls.json` 中。
