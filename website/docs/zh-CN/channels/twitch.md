...
4. 配置令牌：
   - 环境变量：`OpenKrab_TWITCH_ACCESS_TOKEN=...`（仅限默认账户）
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "OpenKrab", // 机器人的 Twitch 账户
      accessToken: "oauth:abc123...", // OAuth Access Token（或使用 OpenKrab_TWITCH_ACCESS_TOKEN 环境变量）
