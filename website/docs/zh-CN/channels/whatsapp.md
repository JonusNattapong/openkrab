...
2. 在 `~/.OpenKrab/OpenKrab.json` 中配置 WhatsApp。
3. 运行 `OpenKrab channels login` 扫描二维码（关联设备）。
4. 启动 Gateway 网关。
...
当设置了 `identity.name` 时，自聊天回复默认为 `[{identity.name}]`（否则为 `[OpenKrab]`），
...
- 登录命令：`OpenKrab channels login`（通过关联设备扫描二维码）。
- 多账户登录：`OpenKrab channels login --account <id>`（`<id>` = `accountId`）。
- 默认账户（省略 `--account` 时）：如果存在则为 `default`，否则为第一个配置的账户 id（排序后）。
- 凭证存储在 `~/.OpenKrab/credentials/whatsapp/<accountId>/creds.json`。
- 备份副本在 `creds.json.bak`（损坏时恢复）。
- 旧版兼容性：较旧的安装将 Baileys 文件直接存储在 `~/.OpenKrab/credentials/` 中。
- 登出：`OpenKrab channels logout`（或 `--account <id>`）删除 WhatsApp 认证状态（但保留共享的 `oauth.json`）。
...
- **私信策略**：`channels.whatsapp.dmPolicy` 控制直接聊天访问（默认：`pairing`）。
  - 配对：未知发送者会收到配对码（通过 `OpenKrab pairing approve whatsapp <code>` 批准；码在 1 小时后过期）。
...
- 使用以下命令批准：`OpenKrab pairing approve whatsapp <code>`（使用 `OpenKrab pairing list whatsapp` 列出）。
...
- 媒体：
  ...
  - 动画 GIF：WhatsApp 期望带有 `gifPlayback: true` 的 MP4 以实现内联循环。
    - CLI：`OpenKrab message send --media <mp4> --gif-playback`
...
- 日志文件：`/tmp/OpenKrab/OpenKrab-YYYY-MM-DD.log`（可配置）。
...
- 修复：在 Gateway 网关主机上运行 `OpenKrab channels login` 并扫描二维码（WhatsApp → 设置 → 关联设备）。

**已关联但断开连接 / 重连循环**

- 症状：`channels status` 显示 `running, disconnected` 或警告"Linked but disconnected"。
- 修复：`OpenKrab doctor`（或重启 Gateway 网关）。如果问题持续，通过 `channels login` 重新关联并检查 `OpenKrab logs --follow`。
