...
- **ä¼šè¯å­˜å‚¨**ï¼ˆèŠå¤©åŽ†å² + è·¯ç”±çŠ¶æ€ï¼‰ä½äºŽ `~/.OpenKrab/agents/<agentId>/sessions` ä¸‹ã€‚
...
Skills é€šè¿‡æ¯ä¸ªå·¥ä½œåŒºçš„ `skills/` æ–‡ä»¶å¤¹å®žçŽ°æ¯æ™ºèƒ½ä½“ç‹¬ç«‹ï¼Œå…±äº«çš„ Skills å¯ä»Ž `~/.OpenKrab/skills` èŽ·å–ã€‚å‚è§ [Skillsï¼šæ¯æ™ºèƒ½ä½“ vs å…±äº«](/tools/skills#per-agent-vs-shared-skills)ã€‚
...
- çŠ¶æ€ç›®å½•ï¼š`~/.OpenKrab`ï¼ˆæˆ– `OPENKRAB_STATE_DIR`ï¼‰
- å·¥ä½œåŒºï¼š`~/.OpenKrab/workspace`ï¼ˆæˆ– `~/.OpenKrab/workspace-<agentId>`ï¼‰
- æ™ºèƒ½ä½“ç›®å½•ï¼š`~/.OpenKrab/agents/<agentId>/agent`ï¼ˆæˆ– `agents.list[].agentDir`ï¼‰
- ä¼šè¯ï¼š`~/.OpenKrab/agents/<agentId>/sessions`

### å•æ™ºèƒ½ä½“æ¨¡å¼ï¼ˆé»˜è®¤ï¼‰

å¦‚æžœä½ ä»€ä¹ˆéƒ½ä¸åšï¼ŒOpenKrab è¿è¡Œå•ä¸ªæ™ºèƒ½ä½“ï¼š

- `agentId` é»˜è®¤ä¸º **`main`**ã€‚
- ä¼šè¯é”®ä¸º `agent:main:<mainKey>`ã€‚
- å·¥ä½œåŒºé»˜è®¤ä¸º `~/.OpenKrab/workspace`ï¼ˆæˆ–å½“è®¾ç½®äº† `OPENKRAB_PROFILE` æ—¶ä¸º `~/.OpenKrab/workspace-<profile>`ï¼‰ã€‚
- çŠ¶æ€é»˜è®¤ä¸º `~/.OpenKrab/agents/main/agent`ã€‚
...
`~/.OpenKrab/OpenKrab.json`ï¼ˆJSON5ï¼‰ï¼š
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // å¯é€‰è¦†ç›–ã€‚é»˜è®¤ï¼š~/.OpenKrab/credentials/whatsapp/personal
