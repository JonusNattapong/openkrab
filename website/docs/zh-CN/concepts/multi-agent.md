...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.OpenKrab/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.OpenKrab/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.OpenKrab`（或 `OpenKrab_STATE_DIR`）
- 工作区：`~/.OpenKrab/workspace`（或 `~/.OpenKrab/workspace-<agentId>`）
- 智能体目录：`~/.OpenKrab/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
...
- 工作区默认为 `~/.OpenKrab/workspace`（或当设置了 `OpenKrab_PROFILE` 时为 `~/.OpenKrab/workspace-<profile>`）。
- 状态默认为 `~/.OpenKrab/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.OpenKrab/credentials/whatsapp/personal
