...
- 在 **Gateway 网关主机**上：
  - 存储文件：`~/.OpenKrab/agents/<agentId>/sessions/sessions.json`（每个智能体）。
- 对话记录：`~/.OpenKrab/agents/<agentId>/sessions/<SessionId>.jsonl`（Telegram 话题会话使用 `.../<SessionId>-topic-<threadId>.jsonl`）。
...
- `OpenKrab status` — 显示存储路径和最近的会话。
- `OpenKrab sessions --json` — 导出每个条目（使用 `--active <minutes>` 过滤）。
- `OpenKrab gateway call sessions.list --params '{}'` — 从运行中的 Gateway 网关获取会话（使用 `--url`/`--token` 进行远程 Gateway 网关访问）。
