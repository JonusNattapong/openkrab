...
summary: ~/.OpenKrab/OpenKrab.json çš„æ‰€æœ‰é…ç½®é€‰é¡¹åŠç¤ºä¾‹
title: é…ç½®
x-i18n:
...
OpenKrab ä»Ž `~/.OpenKrab/OpenKrab.json` è¯»å–å¯é€‰çš„ **JSON5** é…ç½®ï¼ˆæ”¯æŒæ³¨é‡Šå’Œå°¾é€—å·ï¼‰ã€‚

å¦‚æžœæ–‡ä»¶ä¸å­˜åœ¨ï¼ŒOpenKrab ä½¿ç”¨å®‰å…¨çš„é»˜è®¤å€¼ï¼ˆå†…ç½® Pi æ™ºèƒ½ä½“ + æŒ‰å‘é€è€…åˆ†ä¼šè¯ + å·¥ä½œåŒº `~/.OpenKrab/workspace`ï¼‰ã€‚é€šå¸¸åªåœ¨ä»¥ä¸‹æƒ…å†µéœ€è¦é…ç½®ï¼š
...
OpenKrab åªæŽ¥å—å®Œå…¨åŒ¹é… schema çš„é…ç½®ã€‚
æœªçŸ¥é”®ã€ç±»åž‹é”™è¯¯æˆ–æ— æ•ˆå€¼ä¼šå¯¼è‡´ Gateway ç½‘å…³ **æ‹’ç»å¯åŠ¨**ä»¥ç¡®ä¿å®‰å…¨ã€‚

éªŒè¯å¤±è´¥æ—¶ï¼š

- Gateway ç½‘å…³ä¸ä¼šå¯åŠ¨ã€‚
- åªå…è®¸è¯Šæ–­å‘½ä»¤ï¼ˆä¾‹å¦‚ï¼š`OpenKrab doctor`ã€`OpenKrab logs`ã€`OpenKrab health`ã€`OpenKrab status`ã€`OpenKrab service`ã€`OpenKrab help`ï¼‰ã€‚
- è¿è¡Œ `OpenKrab doctor` æŸ¥çœ‹å…·ä½“é—®é¢˜ã€‚
- è¿è¡Œ `OpenKrab doctor --fix`ï¼ˆæˆ– `--yes`ï¼‰åº”ç”¨è¿ç§»/ä¿®å¤ã€‚
...
è¯·ä½¿ç”¨ `config.patch` æˆ– `OpenKrab config set`ã€‚è¯·å¤‡ä»½ `~/.OpenKrab/OpenKrab.json`ã€‚
...
```bash
OpenKrab gateway call config.get --params '{}' # capture payload.hash
OpenKrab gateway call config.apply --params '{
...
```bash
OpenKrab gateway call config.get --params '{}' # capture payload.hash
OpenKrab gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.OpenKrab/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@OpenKrab", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.OpenKrab/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.OpenKrab/workspace" }],
}
```
...
{ "$include": "/etc/OpenKrab/base.json5" } // ç»å¯¹è·¯å¾„
...
```json5
// ~/.OpenKrab/clients/mueller/agents.json5
[
...
```json5
// ~/.OpenKrab/clients/mueller/broadcast.json5
{
...
OpenKrab ä»Žçˆ¶è¿›ç¨‹ï¼ˆshellã€launchd/systemdã€CI ç­‰ï¼‰è¯»å–çŽ¯å¢ƒå˜é‡ã€‚

æ­¤å¤–ï¼Œå®ƒè¿˜ä¼šåŠ è½½ï¼š

- å½“å‰å·¥ä½œç›®å½•ä¸­çš„ `.env`ï¼ˆå¦‚æžœå­˜åœ¨ï¼‰
- `~/.OpenKrab/.env`ï¼ˆå³ `$OPENKRAB_STATE_DIR/.env`ï¼‰ä½œä¸ºå…¨å±€å›žé€€ `.env`
...
å¯é€‰ä¾¿åˆ©åŠŸèƒ½ï¼šå¦‚æžœå¯ç”¨ä¸”é¢„æœŸé”®å‡æœªè®¾ç½®ï¼ŒOpenKrab ä¼šè¿è¡Œä½ çš„ç™»å½• shell å¹¶ä»…å¯¼å…¥ç¼ºå¤±çš„é¢„æœŸé”®ï¼ˆä¸ä¼šè¦†ç›–ï¼‰ã€‚
...
OpenKrab åœ¨ä»¥ä¸‹ä½ç½®å­˜å‚¨**æ¯ä¸ªæ™ºèƒ½ä½“çš„**è®¤è¯é…ç½®æ–‡ä»¶ï¼ˆOAuth + API å¯†é’¥ï¼‰ï¼š

- `<agentDir>/auth-profiles.json`ï¼ˆé»˜è®¤ï¼š`~/.OpenKrab/agents/<agentId>/agent/auth-profiles.json`ï¼‰

å¦è¯·å‚é˜…ï¼š[/concepts/oauth](/concepts/oauth)

æ—§ç‰ˆ OAuth å¯¼å…¥ï¼š

- `~/.OpenKrab/credentials/oauth.json`ï¼ˆæˆ– `$OPENKRAB_STATE_DIR/credentials/oauth.json`ï¼‰
...
- `~/.OpenKrab/agent/*`ï¼ˆç”± `OpenKrab doctor` è¿ç§»åˆ° `~/.OpenKrab/agents/<defaultAgentId>/agent/*`ï¼‰

è¦†ç›–ï¼š

- OAuth ç›®å½•ï¼ˆä»…æ—§ç‰ˆå¯¼å…¥ï¼‰ï¼š`OPENKRAB_OAUTH_DIR`
- æ™ºèƒ½ä½“ç›®å½•ï¼ˆé»˜è®¤æ™ºèƒ½ä½“æ ¹ç›®å½•è¦†ç›–ï¼‰ï¼š`OPENKRAB_AGENT_DIR`ï¼ˆæŽ¨èï¼‰ã€`PI_CODING_AGENT_DIR`ï¼ˆæ—§ç‰ˆï¼‰

é¦–æ¬¡ä½¿ç”¨æ—¶ï¼ŒOpenKrab ä¼šå°† `oauth.json` æ¡ç›®å¯¼å…¥åˆ° `auth-profiles.json` ä¸­ã€‚
...
å¦‚æžœè®¾ç½®äº†ï¼ŒOpenKrab ä¼šæŽ¨å¯¼é»˜è®¤å€¼ï¼ˆä»…åœ¨ä½ æœªæ˜Žç¡®è®¾ç½®æ—¶ï¼‰ï¼š
...
- é»˜è®¤æ—¥å¿—æ–‡ä»¶ï¼š`/tmp/OpenKrab/OpenKrab-YYYY-MM-DD.log`
- å¦‚éœ€ç¨³å®šè·¯å¾„ï¼Œå°† `logging.file` è®¾ä¸º `/tmp/OpenKrab/OpenKrab.log`ã€‚
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // å¯é€‰è¦†ç›–ã€‚é»˜è®¤ï¼š~/.OpenKrab/credentials/whatsapp/biz
...
- æ—§ç‰ˆå•è´¦å· Baileys è®¤è¯ç›®å½•ç”± `OpenKrab doctor` è¿ç§»åˆ° `whatsapp/default`ã€‚
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@OpenKrab"],
        },
      },
    ],
  },
...
- `agents.list[]`ï¼šæ¯æ™ºèƒ½ä½“è¦†ç›–ã€‚
  ...
  - `workspace`ï¼šé»˜è®¤ `~/.OpenKrab/workspace-<agentId>`ï¼ˆå¯¹äºŽ `main`ï¼Œå›žé€€åˆ° `agents.defaults.workspace`ï¼‰ã€‚
  - `agentDir`ï¼šé»˜è®¤ `~/.OpenKrab/agents/<agentId>/agent`ã€‚
...
- `commands.config: true` å¯ç”¨ `/config`ï¼ˆè¯»å†™ `OpenKrab.json`ï¼‰ã€‚
...
OpenKrab ä»…åœ¨å­˜åœ¨ `channels.telegram` é…ç½®æ®µæ—¶å¯åŠ¨ Telegramã€‚æœºå™¨äºº token ä»Ž `channels.telegram.botToken`ï¼ˆæˆ– `channels.telegram.tokenFile`ï¼‰è§£æžï¼Œ`TELEGRAM_BOT_TOKEN` ä½œä¸ºé»˜è®¤è´¦å·çš„å›žé€€ã€‚
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["OpenKrab-dm"], // å¯é€‰ç¾¤ç»„ç§èŠç™½åå•
      },
      guilds: {
        "123456789012345678": {
...
OpenKrab åœ¨æä¾›å•†å¯ç”¨ä¸”ä¸¤ä¸ª token éƒ½å·²è®¾ç½®æ—¶å¯åŠ¨ Slackï¼ˆé€šè¿‡é…ç½®æˆ– `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`ï¼‰ã€‚åœ¨ä¸º cron/CLI å‘½ä»¤æŒ‡å®šæŠ•é€’ç›®æ ‡æ—¶ä½¿ç”¨ `user:<id>`ï¼ˆç§èŠï¼‰æˆ– `channel:<id>`ã€‚
...
è¯·å…ˆå®‰è£…ï¼š`OpenKrab plugins install @OpenKrab/mattermost`ï¼ˆæˆ–ä»Ž git checkout ä½¿ç”¨ `./extensions/mattermost`ï¼‰ã€‚
...
OpenKrab åœ¨è´¦å·å·²é…ç½®ï¼ˆæœºå™¨äºº token + åŸºç¡€ URLï¼‰ä¸”å·²å¯ç”¨æ—¶å¯åŠ¨ Mattermostã€‚token + åŸºç¡€ URL ä»Ž `channels.mattermost.botToken` + `channels.mattermost.baseUrl` æˆ–é»˜è®¤è´¦å·çš„ `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` è§£æžï¼ˆé™¤éž `channels.mattermost.enabled` ä¸º `false`ï¼‰ã€‚
...
OpenKrab ä¼šç”Ÿæˆ `imsg rpc`ï¼ˆé€šè¿‡ stdio çš„ JSON-RPCï¼‰ã€‚æ— éœ€å®ˆæŠ¤è¿›ç¨‹æˆ–ç«¯å£ã€‚
...
åœ¨ç³»ç»Ÿæç¤ºçš„ Runtime è¡Œä¸­æ˜¾ç¤ºçš„å¯é€‰ä»“åº“æ ¹ç›®å½•ã€‚å¦‚æžœæœªè®¾ç½®ï¼ŒOpenKrab ä¼šä»Žå·¥ä½œåŒºï¼ˆå’Œå½“å‰å·¥ä½œç›®å½•ï¼‰å‘ä¸ŠæŸ¥æ‰¾ `.git` ç›®å½•è¿›è¡Œæ£€æµ‹ã€‚è·¯å¾„å¿…é¡»å­˜åœ¨æ‰èƒ½ä½¿ç”¨ã€‚
...
å½“æ–‡ä»¶è¶…è¿‡æ­¤é™åˆ¶æ—¶ï¼ŒOpenKrab ä¼šè®°å½•è­¦å‘Šå¹¶æ³¨å…¥å¸¦æ ‡è®°çš„å¤´å°¾æˆªæ–­å†…å®¹ã€‚
...
è®¾ç½®ç”¨æˆ·æ—¶åŒºç”¨äºŽ**ç³»ç»Ÿæç¤ºä¸Šä¸‹æ–‡**ï¼ˆä¸ç”¨äºŽæ¶ˆæ¯ä¿¡å°ä¸­çš„æ—¶é—´æˆ³ï¼‰ã€‚å¦‚æžœæœªè®¾ç½®ï¼ŒOpenKrab åœ¨è¿è¡Œæ—¶ä½¿ç”¨ä¸»æœºæ—¶åŒºã€‚
...
å¦‚æžœæœªè®¾ç½® `messages.responsePrefix`ï¼Œé»˜è®¤ä¸åº”ç”¨å‰ç¼€ã€‚WhatsApp è‡ªèŠå¤©å›žå¤æ˜¯ä¾‹å¤–ï¼šå®ƒä»¬åœ¨è®¾ç½®æ—¶é»˜è®¤ä¸º `[{identity.name}]`ï¼Œå¦åˆ™ä¸º `[OpenKrab]`ï¼Œä»¥ä¿æŒåŒä¸€æ‰‹æœºä¸Šçš„å¯¹è¯å¯è¯»æ€§ã€‚
...
WhatsApp å…¥ç«™å‰ç¼€é€šè¿‡ `channels.whatsapp.messagePrefix` é…ç½®ï¼ˆå·²å¼ƒç”¨ï¼š`messages.messagePrefix`ï¼‰ã€‚é»˜è®¤ä¿æŒ**ä¸å˜**ï¼šå½“ `channels.whatsapp.allowFrom` ä¸ºç©ºæ—¶ä¸º `"[OpenKrab]"`ï¼Œå¦åˆ™ä¸º `""`ï¼ˆæ— å‰ç¼€ï¼‰ã€‚ä½¿ç”¨ `"[OpenKrab]"` æ—¶ï¼Œå¦‚æžœè·¯ç”±çš„æ™ºèƒ½ä½“è®¾ç½®äº† `identity.name`ï¼ŒOpenKrab ä¼šæ”¹ç”¨ `[{identity.name}]`ã€‚
...
ä¸ºå‡ºç«™å›žå¤å¯ç”¨æ–‡å­—è½¬è¯­éŸ³ã€‚å¼€å¯åŽï¼ŒOpenKrab ä½¿ç”¨ ElevenLabs æˆ– OpenAI ç”ŸæˆéŸ³é¢‘å¹¶é™„åŠ åˆ°å›žå¤ä¸­ã€‚Telegram ä½¿ç”¨ Opus è¯­éŸ³æ¶ˆæ¯ï¼›å…¶ä»–æ¸ é“å‘é€ MP3 éŸ³é¢‘ã€‚
...
`voiceAliases` å…è®¸ Talk æŒ‡ä»¤ä½¿ç”¨å‹å¥½åç§°ï¼ˆä¾‹å¦‚ `"voice":"Krabd"`ï¼‰ã€‚
...
OpenKrab è¿˜å†…ç½®äº†ä¸€äº›åˆ«åå¿«æ·æ–¹å¼ã€‚é»˜è®¤å€¼ä»…åœ¨æ¨¡åž‹å·²å­˜åœ¨äºŽ `agents.defaults.models` ä¸­æ—¶æ‰åº”ç”¨ï¼š
...
å¦‚æžœçœç•¥æä¾›å•†ï¼ŒOpenKrab ç›®å‰å‡å®š `anthropic` ä½œä¸ºä¸´æ—¶å¼ƒç”¨å›žé€€ã€‚
...
- `tools.web.search.apiKey`ï¼ˆæŽ¨èï¼šé€šè¿‡ `OpenKrab configure --section web` è®¾ç½®ï¼Œæˆ–ä½¿ç”¨ `BRAVE_API_KEY` çŽ¯å¢ƒå˜é‡ï¼‰
...
- `group:OpenKrab`ï¼šæ‰€æœ‰å†…ç½® OpenKrab å·¥å…·ï¼ˆä¸åŒ…å«æä¾›å•†æ’ä»¶ï¼‰
...
- æ™ºèƒ½ä½“å·¥ä½œåŒºè®¿é—®ï¼š`workspaceAccess: "none"`ï¼ˆé»˜è®¤ï¼‰
  - `"none"`ï¼šåœ¨ `~/.OpenKrab/sandboxes` ä¸‹ä½¿ç”¨æ¯èŒƒå›´çš„æ²™ç®±å·¥ä½œåŒº
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          containerPrefix: "OpenKrab-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenKrab ä½¿ç”¨ **pi-coding-agent** æ¨¡åž‹ç›®å½•ã€‚ä½ å¯ä»¥é€šè¿‡ç¼–å†™
`~/.OpenKrab/agents/<agentId>/agent/models.json` æˆ–åœ¨ OpenKrab é…ç½®ä¸­çš„ `models.providers` ä¸‹å®šä¹‰ç›¸åŒçš„ schema æ¥æ·»åŠ è‡ªå®šä¹‰æä¾›å•†ï¼ˆLiteLLMã€æœ¬åœ° OpenAI å…¼å®¹æœåŠ¡å™¨ã€Anthropic ä»£ç†ç­‰ï¼‰ã€‚
æŒ‰æä¾›å•†çš„æ¦‚è¿° + ç¤ºä¾‹ï¼š[/concepts/model-providers](/concepts/model-providers)ã€‚

å½“å­˜åœ¨ `models.providers` æ—¶ï¼ŒOpenKrab åœ¨å¯åŠ¨æ—¶å°† `models.json` å†™å…¥/åˆå¹¶åˆ°
`~/.OpenKrab/agents/<agentId>/agent/`ï¼š
...
OpenCode Zen æ˜¯ä¸€ä¸ªå…·æœ‰æ¯æ¨¡åž‹ç«¯ç‚¹çš„å¤šæ¨¡åž‹ç½‘å…³ã€‚OpenKrab ä½¿ç”¨
...
- å¿«æ·æ–¹å¼ï¼š`OpenKrab onboard --auth-choice opencode-zen`ã€‚
...
å¿«æ·æ–¹å¼ï¼š`OpenKrab onboard --auth-choice zai-api-key`ã€‚
...
- åœ¨çŽ¯å¢ƒä¸­è®¾ç½® `MOONSHOT_API_KEY` æˆ–ä½¿ç”¨ `OpenKrab onboard --auth-choice moonshot-api-key`ã€‚
...
- åœ¨çŽ¯å¢ƒä¸­è®¾ç½® `KIMI_API_KEY` æˆ–ä½¿ç”¨ `OpenKrab onboard --auth-choice kimi-code-api-key`ã€‚
...
- è®¾ç½® `SYNTHETIC_API_KEY` æˆ–ä½¿ç”¨ `OpenKrab onboard --auth-choice synthetic-api-key`ã€‚
...
- è®¾ç½® `MINIMAX_API_KEY` çŽ¯å¢ƒå˜é‡æˆ–ä½¿ç”¨ `OpenKrab onboard --auth-choice minimax-api`ã€‚
...
- å¦‚æžœä½ å¸Œæœ› `models.json` å­˜å‚¨åœ¨å…¶ä»–ä½ç½®ï¼Œè¯·ä½¿ç”¨ `OPENKRAB_AGENT_DIR`ï¼ˆæˆ– `PI_CODING_AGENT_DIR`ï¼‰è¦†ç›–æ™ºèƒ½ä½“é…ç½®æ ¹ç›®å½•ï¼ˆé»˜è®¤ï¼š`~/.OpenKrab/agents/main/agent`ï¼‰ã€‚
...
```json5
{
  session: {
    ...
    // é»˜è®¤å·²æŒ‰æ™ºèƒ½ä½“å­˜å‚¨åœ¨ ~/.OpenKrab/agents/<agentId>/sessions/sessions.json
...
- `resetByType`ï¼š`dm`ã€`group` å’Œ `thread` çš„æ¯ä¼šè¯è¦†ç›–ã€‚
  - å¦‚æžœä½ åªè®¾ç½®äº†æ—§ç‰ˆ `session.idleMinutes` è€Œæ²¡æœ‰ä»»ä½• `reset`/`resetByType`ï¼ŒOpenKrab ä¿æŒä»…ç©ºé—²æ¨¡å¼ä»¥å‘åŽå…¼å®¹ã€‚
...
æŽ§åˆ¶å†…ç½®ç™½åå•ã€å®‰è£…åå¥½ã€é¢å¤– Skills æ–‡ä»¶å¤¹å’Œæ¯ Skills è¦†ç›–ã€‚é€‚ç”¨äºŽ**å†…ç½®**Skills å’Œ `~/.OpenKrab/skills`ï¼ˆå·¥ä½œåŒº Skills åœ¨åç§°å†²çªæ—¶ä»ç„¶ä¼˜å…ˆï¼‰ã€‚
...
æŽ§åˆ¶æ’ä»¶å‘çŽ°ã€å…è®¸/æ‹’ç»å’Œæ¯æ’ä»¶é…ç½®ã€‚æ’ä»¶ä»Ž `~/.OpenKrab/extensions`ã€`<workspace>/.OpenKrab/extensions` ä»¥åŠä»»ä½• `plugins.load.paths` æ¡ç›®åŠ è½½ã€‚**é…ç½®æ›´æ”¹éœ€è¦é‡å¯ Gateway ç½‘å…³ã€‚**
...
### `browser`ï¼ˆOpenKrab ç®¡ç†çš„æµè§ˆå™¨ï¼‰

OpenKrab å¯ä»¥ä¸º OpenKrab å¯åŠ¨ä¸€ä¸ª**ä¸“ç”¨ã€éš”ç¦»çš„** Chrome/Brave/Edge/Chromium å®žä¾‹å¹¶æš´éœ²ä¸€ä¸ªå°åž‹ local loopback æŽ§åˆ¶æœåŠ¡ã€‚
...
- æ³¨æ„ï¼šæŽ§åˆ¶æœåŠ¡å™¨ç”±è¿è¡Œä¸­çš„ Gateway ç½‘å…³ï¼ˆOpenKrab.app èœå•æ æˆ– `OpenKrab gateway`ï¼‰å¯åŠ¨ã€‚
...
- ç¤ºä¾‹ï¼š`"/ui"`ã€`"/OpenKrab"`ã€`"/apps/OpenKrab"`ã€‚
...
- å½“è¿žæŽ¥æ¥è‡ªè¿™äº› IP ä¹‹ä¸€æ—¶ï¼ŒOpenKrab ä½¿ç”¨ `x-forwarded-for`ï¼ˆæˆ– `x-real-ip`ï¼‰æ¥ç¡®å®šå®¢æˆ·ç«¯ IPï¼Œç”¨äºŽæœ¬åœ°é…å¯¹æ£€æŸ¥å’Œ HTTP è®¤è¯/æœ¬åœ°æ£€æŸ¥ã€‚
- ä»…åˆ—å‡ºä½ å®Œå…¨æŽ§åˆ¶çš„ä»£ç†ï¼Œå¹¶ç¡®ä¿å®ƒä»¬**è¦†ç›–**ä¼ å…¥çš„ `x-forwarded-for`ã€‚

è¯´æ˜Žï¼š

- `OpenKrab gateway` æ‹’ç»å¯åŠ¨ï¼Œé™¤éž `gateway.mode` è®¾ä¸º `local`ï¼ˆæˆ–ä½ ä¼ é€’äº†è¦†ç›–æ ‡å¿—ï¼‰ã€‚
...
- ä¼˜å…ˆçº§ï¼š`--port` > `OPENKRAB_GATEWAY_PORT` > `gateway.port` > é»˜è®¤ `18789`ã€‚
...
- `gateway.auth.password` å¯åœ¨æ­¤è®¾ç½®ï¼Œæˆ–é€šè¿‡ `OPENKRAB_GATEWAY_PASSWORD`ï¼ˆæŽ¨èï¼‰ã€‚
- `gateway.auth.allowTailscale` å…è®¸ Tailscale Serve èº«ä»½å¤´éƒ¨
  ...
  `x-forwarded-proto` å’Œ `x-forwarded-host` æ—¶æ»¡è¶³è®¤è¯ã€‚OpenKrab åœ¨æŽ¥å—ä¹‹å‰
...
- OpenKrab.app ç›‘è§† `~/.OpenKrab/OpenKrab.json`ï¼Œå½“ `gateway.mode` æˆ– `gateway.remote.url` å˜æ›´æ—¶å®žæ—¶åˆ‡æ¢æ¨¡å¼ã€‚
...
Gateway ç½‘å…³ç›‘è§† `~/.OpenKrab/OpenKrab.json`ï¼ˆæˆ– `OPENKRAB_CONFIG_PATH`ï¼‰å¹¶è‡ªåŠ¨åº”ç”¨æ›´æ”¹ã€‚
...
- `~/.OpenKrab/OpenKrab.json`ï¼ˆæˆ– `OPENKRAB_CONFIG_PATH`ï¼‰
...
- `OPENKRAB_STATE_DIR`ï¼ˆä¼šè¯/å‡­æ®ï¼‰
- `agents.defaults.workspace`ï¼ˆè®°å¿†ï¼‰
- `gateway.port`ï¼ˆæ¯å®žä¾‹å”¯ä¸€ï¼‰

ä¾¿åˆ©æ ‡å¿—ï¼ˆCLIï¼‰ï¼š

- `OpenKrab --dev â€¦` â†’ ä½¿ç”¨ `~/.OpenKrab-dev` + ç«¯å£ä»ŽåŸºç¡€ `19001` åç§»
- `OpenKrab --profile <name> â€¦` â†’ ä½¿ç”¨ `~/.OpenKrab-<name>`ï¼ˆç«¯å£é€šè¿‡é…ç½®/çŽ¯å¢ƒå˜é‡/æ ‡å¿—ï¼‰
...
Gmail è¾…åŠ©é…ç½®ï¼ˆç”± `OpenKrab webhooks gmail setup` / `run` ä½¿ç”¨ï¼‰ï¼š
...
- è®¾ç½® `OPENKRAB_SKIP_GMAIL_WATCHER=1` ç¦ç”¨è‡ªåŠ¨å¯åŠ¨ï¼ˆç”¨äºŽæ‰‹åŠ¨è¿è¡Œï¼‰ã€‚
...
æ³¨æ„ï¼šå½“ `tailscale.mode` å¼€å¯æ—¶ï¼ŒOpenKrab å°† `serve.path` é»˜è®¤ä¸º `/`ï¼Œä»¥ä¾¿
...
é»˜è®¤æ ¹ç›®å½•ï¼š`~/.OpenKrab/workspace/canvas`
é»˜è®¤ç«¯å£ï¼š`18793`ï¼ˆé€‰æ‹©æ­¤ç«¯å£ä»¥é¿å… OpenKrab æµè§ˆå™¨ CDP ç«¯å£ `18792`ï¼‰
...
- ç›‘è§†ç›®å½•å¹¶é€šè¿‡ `/__OPENKRAB__/ws` çš„ WebSocket ç«¯ç‚¹å¹¿æ’­é‡è½½
- ç›®å½•ä¸ºç©ºæ—¶è‡ªåŠ¨åˆ›å»ºèµ·å§‹ `index.html`ï¼ˆä»¥ä¾¿ä½ ç«‹å³çœ‹åˆ°å†…å®¹ï¼‰
- åŒæ—¶åœ¨ `/__OPENKRAB__/a2ui/` æä¾› A2UIï¼Œå¹¶ä½œä¸º `canvasHostUrl` é€šå‘Šç»™èŠ‚ç‚¹
...
- çŽ¯å¢ƒå˜é‡ï¼š`OPENKRAB_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // çœç•¥æ—¶ä½¿ç”¨ ~/.OpenKrab/bridge/tls/bridge-{cert,key}.pemã€‚
      // certPath: "~/.OpenKrab/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.OpenKrab/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
æŽ§åˆ¶ LAN mDNS å‘çŽ°å¹¿æ’­ï¼ˆ`_OpenKrab-gw._tcp`ï¼‰ã€‚

- `minimal`ï¼ˆé»˜è®¤ï¼‰ï¼šä»Ž TXT è®°å½•ä¸­çœç•¥ `cliPath` + `sshPort`
- `full`ï¼šåœ¨ TXT è®°å½•ä¸­åŒ…å« `cliPath` + `sshPort`
- `off`ï¼šå®Œå…¨ç¦ç”¨ mDNS å¹¿æ’­
- ä¸»æœºåï¼šé»˜è®¤ä¸º `OpenKrab`ï¼ˆé€šå‘Š `OpenKrab.local`ï¼‰ã€‚é€šè¿‡ `OPENKRAB_MDNS_HOSTNAME` è¦†ç›–ã€‚
...
å¯ç”¨åŽï¼ŒGateway ç½‘å…³åœ¨ `~/.OpenKrab/dns/` ä¸‹ä½¿ç”¨é…ç½®çš„å‘çŽ°åŸŸï¼ˆç¤ºä¾‹ï¼š`OpenKrab.internal.`ï¼‰ä¸º `_OpenKrab-gw._tcp` å†™å…¥å•æ’­ DNS-SD åŒºåŸŸã€‚
