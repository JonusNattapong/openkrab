...
read_when:
  - 添加或修改配置字段时
summary: ~/.OpenKrab/OpenKrab.json 的所有配置选项及示例
title: 配置
x-i18n:
...
OpenKrab 从 `~/.OpenKrab/OpenKrab.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，OpenKrab 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.OpenKrab/workspace`）。通常只在以下情况需要配置：
...
- Gateway 网关不会启动。
- 只允许诊断命令（例如：`OpenKrab doctor`、`OpenKrab logs`、`OpenKrab health`、`OpenKrab status`、`OpenKrab service`、`OpenKrab help`）。
- 运行 `OpenKrab doctor` 查看具体问题。
- 运行 `OpenKrab doctor --fix`（或 `--yes`）应用迁移/修复。
...
请使用 `config.patch` 或 `OpenKrab config set`。请备份 `~/.OpenKrab/OpenKrab.json`。
...
{ "$include": "/etc/OpenKrab/base.json5" } // 绝对路径
...
- 当前工作目录中的 `.env`（如果存在）
- `~/.OpenKrab/.env`（即 `$OpenKrab_STATE_DIR/.env`）作为全局回退 `.env`
...
- `<agentDir>/auth-profiles.json`（默认：`~/.OpenKrab/agents/<agentId>/agent/auth-profiles.json`）

另请参阅：[/concepts/oauth](/concepts/oauth)

旧版 OAuth 导入：

- `~/.OpenKrab/credentials/oauth.json`（或 `$OpenKrab_STATE_DIR/credentials/oauth.json`）
...
- `~/.OpenKrab/agent/*`（由 `OpenKrab doctor` 迁移到 `~/.OpenKrab/agents/<defaultAgentId>/agent/*`）

覆盖：

- OAuth 目录（仅旧版导入）：`OpenKrab_OAUTH_DIR`
- 智能体目录（默认智能体根目录覆盖）：`OpenKrab_AGENT_DIR`（推荐）、`PI_CODING_AGENT_DIR`（旧版）
...
- 默认日志文件：`/tmp/OpenKrab/OpenKrab-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/OpenKrab/OpenKrab.log`。
- 控制台输出可通过以下方式单独调整：
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // 可选覆盖。默认：~/.OpenKrab/credentials/whatsapp/biz
...
- 旧版单账号 Baileys 认证目录由 `OpenKrab doctor` 迁移到 `whatsapp/default`。
...
- `agents.list[]`：每智能体覆盖。
  ...
  - `name`：智能体的显示名称。
  - `workspace`：默认 `~/.OpenKrab/workspace-<agentId>`（对于 `main`，回退到 `agents.defaults.workspace`）。
  - `agentDir`：默认 `~/.OpenKrab/agents/<agentId>/agent`。
...
- `commands.config: true` 启用 `/config`（读写 `OpenKrab.json`）。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["OpenKrab-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
...
请先安装：`OpenKrab plugins install @OpenKrab/mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[OpenKrab]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[OpenKrab]"`，否则为 `""`（无前缀）。使用 `"[OpenKrab]"` 时，如果路由的智能体设置了 `identity.name`，OpenKrab 会改用 `[{identity.name}]`。
...
- `tools.web.search.apiKey`（推荐：通过 `OpenKrab configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
- `group:nodes`：`nodes`
- `group:OpenKrab`：所有内置 OpenKrab 工具（不包含提供商插件）
...
- 智能体工作区访问：`workspaceAccess: "none"`（默认）
  - `"none"`：在 `~/.OpenKrab/sandboxes` 下使用每范围的沙箱工作区
...
`~/.OpenKrab/agents/<agentId>/agent/models.json` 或在 OpenKrab 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，OpenKrab 在启动时将 `models.json` 写入/合并到
`~/.OpenKrab/agents/<agentId>/agent/`：
...
- 快捷方式：`OpenKrab onboard --auth-choice opencode-zen`。
...
快捷方式：`OpenKrab onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `OpenKrab onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `OpenKrab onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `OpenKrab onboard --auth-choice synthetic-api-key`。
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `OpenKrab onboard --auth-choice minimax-api`。
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `OpenKrab_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.OpenKrab/agents/main/agent`）。
...
```json5
{
  session: {
    ...
    // 默认已按智能体存储在 ~/.OpenKrab/agents/<agentId>/sessions/sessions.json
...
控制内置白名单、安装偏好、额外 Skills 文件夹和每 Skills 覆盖。适用于**内置**Skills 和 `~/.OpenKrab/skills`（工作区 Skills 在名称冲突时仍然优先）。
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.OpenKrab/extensions`、`<workspace>/.OpenKrab/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway 网关。**
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（OpenKrab.app 菜单栏或 `OpenKrab gateway`）启动。
...
- `OpenKrab gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- 优先级：`--port` > `OpenKrab_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- `gateway.auth.password` 可在此设置，或通过 `OpenKrab_GATEWAY_PASSWORD`（推荐）。
...
- OpenKrab.app 监视 `~/.OpenKrab/OpenKrab.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.OpenKrab/OpenKrab.json`（或 `OpenKrab_CONFIG_PATH`）并自动应用更改。
...
- `~/.OpenKrab/OpenKrab.json`（或 `OpenKrab_CONFIG_PATH`）
...
- `OpenKrab_STATE_DIR`（会话/凭据）
- `agents.defaults.workspace`（记忆）
- `gateway.port`（每实例唯一）

便利标志（CLI）：

- `OpenKrab --dev …` → 使用 `~/.OpenKrab-dev` + 端口从基础 `19001` 偏移
- `OpenKrab --profile <name> …` → 使用 `~/.OpenKrab-<name>`（端口通过配置/环境变量/标志）
...
Gmail 辅助配置（由 `OpenKrab webhooks gmail setup` / `run` 使用）：
...
- 设置 `OpenKrab_SKIP_GMAIL_WATCHER=1` 禁用自动启动（用于手动运行）。
...
默认根目录：`~/.OpenKrab/workspace/canvas`
...
- 监视目录并通过 `/__OpenKrab__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__OpenKrab__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
- 环境变量：`OpenKrab_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // 省略时使用 ~/.OpenKrab/bridge/tls/bridge-{cert,key}.pem。
...
控制 LAN mDNS 发现广播（`_OpenKrab-gw._tcp`）。

- `minimal`（默认）：从 TXT 记录中省略 `cliPath` + `sshPort`
- `full`：在 TXT 记录中包含 `cliPath` + `sshPort`
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `OpenKrab`（通告 `OpenKrab.local`）。通过 `OpenKrab_MDNS_HOSTNAME` 覆盖。
...
启用后，Gateway 网关在 `~/.OpenKrab/dns/` 下使用配置的发现域（示例：`OpenKrab.internal.`）为 `_OpenKrab-gw._tcp` 写入单播 DNS-SD 区域。
