...
设计目标是将所有网络发现/广播保留在 **Node Gateway 网关**（`OpenKrab gateway`）中，并让客户端（mac 应用、iOS）作为消费者。
...
- 服务类型：
  - `_OpenKrab-gw._tcp`（Gateway 网关传输信标）
- TXT 键（非机密）：
  - `role=gateway`
  ...
  - `canvasPort=18793`（默认画布主机端口；服务于 `/__OpenKrab__/canvas/`）
  - `cliPath=<path>`（可选；可运行的 `OpenKrab` 入口点或二进制文件的绝对路径）
  - `tailnetDns=<magicdns>`（可选提示；当 Tailscale 可用时自动检测）

禁用/覆盖：

- `OpenKrab_DISABLE_BONJOUR=1` 禁用广播。
- `~/.OpenKrab/OpenKrab.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `OpenKrab_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（默认为 22）。
- `OpenKrab_TAILNET_DNS` 发布 `tailnetDns` 提示（MagicDNS）。
- `OpenKrab_CLI_PATH` 覆盖广播的 CLI 路径。
