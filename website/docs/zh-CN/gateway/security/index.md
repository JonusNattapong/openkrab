...
- 收紧本地权限（`~/.OpenKrab` → `700`，配置文件 → `600`，以及常见状态文件如 `credentials/*.json`、`agents/*/agent/auth-profiles.json` 和 `agents/*/sessions/sessions.json`）。
...
- **配对白名单**：`~/.OpenKrab/credentials/<channel>-allowFrom.json`
- **模型认证配置**：`~/.OpenKrab/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.OpenKrab/credentials/oauth.json`
...
`OpenKrab security audit` 会在启用此设置时发出警告。
...
OpenKrab 将会话记录存储在 `~/.OpenKrab/agents/<agentId>/sessions/*.jsonl` 下的磁盘上。这是会话连续性和（可选）会话记忆索引所必需的，但这也意味着**任何具有文件系统访问权限的进程/用户都可以读取这些日志**。将磁盘访问视为信任边界，并锁定 `~/.OpenKrab` 的权限（参见下面的审计部分）。如果你需要在智能体之间进行更强的隔离，请在单独的操作系统用户或单独的主机下运行它们。
...
- 在启用之前审查插件配置。
- 在插件更改后重启 Gateway 网关。
- 如果你从 npm 安装插件（`OpenKrab plugins install <npm-spec>`），将其视为运行不受信任的代码：
  - 安装路径是 `~/.OpenKrab/extensions/<pluginId>/`（或 `$OpenKrab_STATE_DIR/extensions/<pluginId>/`）。
...
- **私信白名单**（`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`）：谁被允许在私信中与机器人交谈。
  - 当 `dmPolicy="pairing"` 时，批准会写入 `~/.OpenKrab/credentials/<channel>-allowFrom.json`（与配置白名单合并）。
...
4. **重新运行审计**
   - `OpenKrab security audit --deep` 并确认报告是干净的。
...
- `~/.OpenKrab/OpenKrab.json`：`600`（仅用户读/写）
- `~/.OpenKrab`：`700`（仅用户）

`OpenKrab doctor` 可以警告并提供收紧这些权限的选项。
...
- 默认：`18789`
- 配置/标志/环境变量：`gateway.port`、`--port`、`OpenKrab_GATEWAY_PORT`
...
Gateway 网关通过 mDNS（端口 5353 上的 `_OpenKrab-gw._tcp`）广播其存在以用于本地设备发现。在完整模式下，这包括可能暴露运营详情的 TXT 记录：
...
4. **环境变量**（替代方案）：设置 `OpenKrab_DISABLE_BONJOUR=1` 以在不更改配置的情况下禁用 mDNS。
...
Doctor 可以为你生成一个：`OpenKrab doctor --generate-gateway-token`。
...
- `gateway.auth.mode: "password"`：密码认证（优先通过环境变量设置：`OpenKrab_GATEWAY_PASSWORD`）。

轮换清单（令牌/密码）：

1. 生成/设置一个新的秘密（`gateway.auth.token` 或 `OpenKrab_GATEWAY_PASSWORD`）。
...
假设 `~/.OpenKrab/`（或 `$OpenKrab_STATE_DIR/`）下的任何内容都可能包含秘密或私有数据：

- `OpenKrab.json`：配置可能包含令牌（Gateway 网关、远程 Gateway 网关）、提供商设置和白名单。
...
- 共享诊断信息时，优先使用 `OpenKrab status --all`（可粘贴，秘密已脱敏）而不是原始日志。
...
- `agents.defaults.sandbox.workspaceAccess: "none"`（默认）使智能体工作区不可访问；工具针对 `~/.OpenKrab/sandboxes` 下的沙箱工作区运行
...
- 优先为智能体使用专用配置文件（默认的 `OpenKrab` 配置文件）。
...
1. **停止它：** 停止 macOS 应用（如果它监督 Gateway 网关）或终止你的 `OpenKrab gateway` 进程。
...
1. 轮换 Gateway 网关认证（`gateway.auth.token` / `OpenKrab_GATEWAY_PASSWORD`）并重启。
...
1. 检查 Gateway 网关日志：`/tmp/OpenKrab/OpenKrab-YYYY-MM-DD.log`（或 `logging.file`）。
2. 审查相关记录：`~/.OpenKrab/agents/<agentId>/sessions/*.jsonl`。
