...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [安装和设置 OpenKrab 的推荐方式是什么？](#whats-the-recommended-way-to-install-and-set-up-OpenKrab)
  ...
  - [无法访问 docs.OpenKrab.ai（SSL 错误），怎么办？](#i-cant-access-docsOpenKrabai-ssl-error-what-now)
...
- [如何安装 beta 版本，beta 和 dev 有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Windows 安装提示找不到 git 或无法识别 OpenKrab](#windows-install-says-git-not-found-or-OpenKrab-not-recognized)
  - [文档没有解答我的问题——如何获得更好的答案？](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [如何在 Linux 上安装 OpenKrab？](#how-do-i-install-OpenKrab-on-linux)
  - [如何在 VPS 上安装 OpenKrab？](#how-do-i-install-OpenKrab-on-a-vps)
  - [云/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [可以让 OpenKrab 自行更新吗？](#can-i-ask-OpenKrab-to-update-itself)
  ...
  - [如果我买了 Mac mini 运行 OpenKrab，能连接到我的 MacBook Pro 吗？](#if-i-buy-a-mac-mini-to-run-OpenKrab-can-i-connect-it-to-my-macbook-pro)
  - [可以使用 Bun 吗？](#can-i-use-bun)
  - [Telegram：`allowFrom` 填什么？](#telegram-what-goes-in-allowfrom)
  - [多人能否使用同一个 WhatsApp 号码配合不同的 OpenKrab 实例？](#can-multiple-people-use-one-whatsapp-number-with-different-OpenKrab-instances)
  ...
  - [在专用机器上运行 OpenKrab 有多重要？](#how-important-is-it-to-run-OpenKrab-on-a-dedicated-machine)
  - [VPS 的最低要求和推荐操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [可以在虚拟机中运行 OpenKrab 吗？有什么要求？](#can-i-run-OpenKrab-in-a-vm-and-what-are-the-requirements)
- [什么是 OpenKrab？](#what-is-OpenKrab)
  - [用一段话描述 OpenKrab？](#what-is-OpenKrab-in-one-paragraph)
  - [价值主张是什么？](#whats-the-value-proposition)
  - [刚设置好，应该先做什么？](#i-just-set-it-up-what-should-i-do-first)
  - [OpenKrab 日常最常用的五个场景是什么？](#what-are-the-top-five-everyday-use-cases-for-OpenKrab)
  - [OpenKrab 能否帮助 SaaS 进行获客、外联、广告和博客？](#can-OpenKrab-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills 与自动化](#skills-and-automation)
  ...
  - [OpenKrab 能否按计划或在后台持续运行任务？](#can-OpenKrab-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [磁盘上的文件位置](#where-things-live-on-disk)
  - [OpenKrab 使用的所有数据都保存在本地吗？](#is-all-data-used-with-OpenKrab-saved-locally)
  - [OpenKrab 将数据存储在哪里？](#where-does-OpenKrab-store-its-data)
  ...
  - [如何完全卸载 OpenKrab？](#how-do-i-completely-uninstall-OpenKrab)
...
- [配置基础](#config-basics)
  ...
  - [OpenKrab 浏览器可以无头运行吗？](#can-the-OpenKrab-browser-run-headless)
...
- [远程 Gateway 网关与节点](#remote-gateways-nodes)
  ...
  - [两个 OpenKrab 实例（本地 + VPS）可以互相通信吗？](#can-two-OpenKrab-instances-talk-to-each-other-local-vps)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [OpenKrab 如何加载环境变量？](#how-does-OpenKrab-load-environment-variables)
...
- [会话与多聊天](#sessions-multiple-chats)
  ...
  - [能否创建一个 OpenKrab 实例团队——一个 CEO 和多个智能体？](#is-there-a-way-to-make-a-team-of-OpenKrab-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断了？如何防止？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 OpenKrab 但保留安装？](#how-do-i-completely-reset-OpenKrab-but-keep-it-installed)
  ...
  - [为什么 OpenKrab 不在群组中回复？](#why-doesnt-OpenKrab-reply-in-a-group)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [OpenKrab、Flawd 和 Krill 使用什么模型？](#what-do-OpenKrab-flawd-and-krill-use-for-models)
...
- [Gateway 网关：端口、“已在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [为什么 `OpenKrab gateway status` 显示 `Runtime: running` 但 `RPC probe: failed`？](#why-does-OpenKrab-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [为什么 `OpenKrab gateway status` 显示 `Config (cli)` 和 `Config (service)` 不同？](#why-does-OpenKrab-gateway-status-show-config-cli-and-config-service-different)
  - ["another gateway instance is already listening"是什么意思？](#what-does-another-gateway-instance-is-already-listening-mean)
  - [如何以远程模式运行 OpenKrab（客户端连接到其他位置的 Gateway 网关）？](#how-do-i-run-OpenKrab-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [日志与调试](#logging-and-debugging)
  ...
  - [我在 Windows 上关闭了终端——如何重启 OpenKrab？](#i-closed-my-terminal-on-windows-how-do-i-restart-OpenKrab)
  ...
  - [通俗解释：`OpenKrab gateway restart` 与 `OpenKrab gateway`](#eli5-OpenKrab-gateway-restart-vs-OpenKrab-gateway)
...
- [安全与访问控制](#security-and-access-control)
  - [将 OpenKrab 暴露给入站私信安全吗？](#is-it-safe-to-expose-OpenKrab-to-inbound-dms)
...
7. **Gateway 网关快照**
   ...
   OpenKrab health --verbose   # 出错时显示目标 URL + 配置路径
   ```
...
- `OpenKrab status`：Gateway 网关/智能体健康状况 + 基本配置的快速快照。
- `OpenKrab models status`：检查提供商认证 + 模型可用性。
- `OpenKrab doctor`：验证并修复常见的配置/状态问题。

其他有用的 CLI 检查：`OpenKrab status --all`、`OpenKrab logs --follow`、
...
如果你还没有全局安装，通过 `pnpm OpenKrab onboard` 运行。
...
- 如果要求认证，运行 `OpenKrab dashboard` 并使用带令牌的链接（`?token=...`）。
- 令牌与 `gateway.auth.token`（或 `OpenKrab_GATEWAY_TOKEN`）的值相同，UI 在首次加载后会存储它。

**非本地环境：**

- **Tailscale Serve**（推荐）：保持绑定 loopback，运行 `OpenKrab gateway --tailscale serve`，打开 `https://<magicdns>/`。如果 `gateway.auth.allowTailscale` 为 `true`，身份标头满足认证要求（无需令牌）。
- **Tailnet 绑定**：运行 `OpenKrab gateway --bind tailnet --token "<token>"`，打开 `http://<tailscale-ip>:18789/`，在仪表板设置中粘贴令牌。
- **SSH 隧道**：`ssh -N -L 18789:127.0.0.1:18789 user@host`，然后从 `OpenKrab dashboard` 打开 `http://127.0.0.1:18789/?token=...`。
...
1. 在新机器上安装 OpenKrab。
2. 从旧机器复制 `$OpenKrab_STATE_DIR`（默认：`~/.OpenKrab`）。
3. 复制你的工作区（默认：`~/.OpenKrab/workspace`）。
4. 运行 `OpenKrab doctor` 并重启 Gateway 网关服务。

这会保留配置、认证配置文件、WhatsApp 凭据、会话和记忆。如果你处于远程模式，请记住 Gateway 网关主机拥有会话存储和工作区。

**重要：** 如果你只将工作区提交/推送到 GitHub，你只备份了**记忆 + 引导文件**，但**不包括**会话历史或认证。它们位于 `~/.OpenKrab/` 下（例如 `~/.OpenKrab/agents/<agentId>/sessions/`）。

相关：[迁移](/install/migrating)、[磁盘上的文件位置](/help/faq#where-does-OpenKrab-store-its-data)、
...
### 无法访问 docs.OpenKrab.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.OpenKrab.ai`。禁用该功能或将 `docs.OpenKrab.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsOpenKrabai-shows-an-ssl-error-comcastxfinity)。
...
### Windows 安装提示找不到 git 或无法识别 OpenKrab
...
**2) OpenKrab is not recognized（安装后）**
...
`OpenKrab onboard` 是推荐的设置路径。在**本地模式**下，它引导你完成：
...
`claude setup-token` 通过 Claude Code CLI 生成一个**令牌字符串**（在 Web 控制台中不可用）。你可以在**任何机器**上运行它。在向导中选择 **Anthropic token (paste setup-token)** 或使用 `OpenKrab models auth paste-token --provider anthropic` 粘贴。令牌作为 **anthropic** 提供商的认证配置文件存储，像 API 密钥一样使用（无自动刷新）。更多详情：[OAuth](/concepts/oauth)。
...
复制它打印的令牌，然后在向导中选择 **Anthropic token (paste setup-token)**。如果你想在 Gateway 网关主机上运行，使用 `OpenKrab models auth setup-token --provider anthropic`。如果你在其他地方运行了 `claude setup-token`，在 Gateway 网关主机上使用 `OpenKrab models auth paste-token --provider anthropic` 粘贴。参阅 [Anthropic](/providers/anthropic)。
...
Gemini CLI 使用**插件认证流程**，而不是 `OpenKrab.json` 中的 client id 或 secret。

步骤：

1. 启用插件：`OpenKrab plugins enable google-gemini-cli-auth`
2. 登录：`OpenKrab models auth login --provider google-gemini-cli --set-default`
...
- 使用 `OpenKrab nodes status` / `OpenKrab nodes list` 查看它。
...
- 给你的机器人发私信，然后运行 `OpenKrab logs --follow` 并读取 `from.id`。
...
（`~/.OpenKrab`）和工作区（`~/.OpenKrab/workspace`）保持不变。
...
展示：https://OpenKrab.ai/showcase

## Skills 与自动化

### 如何自定义 Skills 而不弄脏仓库

使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.OpenKrab/skills/<name>/SKILL.md`（或通过 `~/.OpenKrab/OpenKrab.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.OpenKrab/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.OpenKrab/OpenKrab.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.OpenKrab/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，OpenKrab 将其视为 `<workspace>/skills`。
...
- 确认 cron 已启用（`cron.enabled`）且未设置 `OpenKrab_SKIP_CRON`。
...
不能直接运行。macOS Skills 受 `metadata.OpenKrab.os` 和所需二进制文件限制，Skills 只有在 **Gateway 网关主机**上符合条件时才会出现在系统提示中。在 Linux 上，`darwin` 专用 Skills（如 `apple-notes`、`apple-reminders`、`things-mac`）不会加载，除非你覆盖限制。

你有三种支持的模式：

**方案 A - 在 Mac 上运行 Gateway 网关（最简单）。**
在 macOS 二进制文件所在的地方运行 Gateway 网关，然后从 Linux 通过[远程模式](#how-do-i-run-OpenKrab-in-remote-mode-client-connects-to-a-gateway-elsewhere)或 Tailscale 连接。Skills 正常加载，因为 Gateway 网关主机是 macOS。
...
3. 覆盖 Skills 元数据（工作区或 `~/.OpenKrab/skills`）以允许 Linux：
   ```markdown
   ---
   name: imsg
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 OpenKrab 工作区）；OpenKrab 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.OpenKrab/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [ClawHub](/tools/clawhub)。
...
- **默认本地：** 会话、记忆文件、配置和工作区位于 Gateway 网关主机上（`~/.OpenKrab` + 你的工作区目录）。
...
所有内容位于 `$OpenKrab_STATE_DIR`（默认：`~/.OpenKrab`）下：

| 路径                                                            | 用途                                                 |
| --------------------------------------------------------------- | ---------------------------------------------------- |
| `$OpenKrab_STATE_DIR/OpenKrab.json`                             | 主配置（JSON5）                                      |
| `$OpenKrab_STATE_DIR/credentials/oauth.json`                    | 旧版 OAuth 导入（首次使用时复制到认证配置文件）      |
| `$OpenKrab_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | 认证配置文件（OAuth + API 密钥）                     |
| `$OpenKrab_STATE_DIR/agents/<agentId>/agent/auth.json`          | 运行时认证缓存（自动管理）                           |
| `$OpenKrab_STATE_DIR/credentials/`                              | 提供商状态（例如 `whatsapp/<accountId>/creds.json`） |
| `$OpenKrab_STATE_DIR/agents/`                                   | 按智能体的状态（agentDir + 会话）                    |
| `$OpenKrab_STATE_DIR/agents/<agentId>/sessions/`                | 对话历史和状态（按智能体）                           |
| `$OpenKrab_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | 会话元数据（按智能体）                               |

旧版单智能体路径：`~/.OpenKrab/agent/*`（通过 `OpenKrab doctor` 迁移）。

你的**工作区**（AGENTS.md、记忆文件、Skills 等）是独立的，通过 `agents.defaults.workspace` 配置（默认：`~/.OpenKrab/workspace`）。

### AGENTS.md / SOUL.md / USER.md / MEMORY.md 应该放在哪里

这些文件位于**智能体工作区**中，而不是 `~/.OpenKrab`。

- **工作区（按智能体）**：`AGENTS.md`、`SOUL.md`、`IDENTITY.md`、`USER.md`、
  `MEMORY.md`（或 `memory.md`）、`memory/YYYY-MM-DD.md`、可选的 `HEARTBEAT.md`。
- **状态目录（`~/.OpenKrab`）**：配置、凭据、认证配置文件、会话、日志和共享 Skills（`~/.OpenKrab/skills`）。

默认工作区是 `~/.OpenKrab/workspace`，可通过以下方式配置：
...
**不要**提交 `~/.OpenKrab` 下的任何内容（凭据、会话、令牌）。如果你需要完整恢复，将工作区和状态目录分别备份（参阅上面的迁移问题）。
...
OpenKrab 从 `$OpenKrab_CONFIG_PATH`（默认：`~/.OpenKrab/OpenKrab.json`）读取可选的 **JSON5** 配置：

```
$OpenKrab_CONFIG_PATH
```

如果文件不存在，使用安全的默认值（包括默认工作区 `~/.OpenKrab/workspace`）。

### 我设置了 gateway.bind: "lan"（或 "tailnet"），现在什么都监听不了 / UI 显示未授权

非 local loopback 绑定**需要认证**。配置 `gateway.auth.mode` + `gateway.auth.token`（或使用 `OpenKrab_GATEWAY_TOKEN`）。
...
如果你**确实**想要开放 local loopback，从配置中移除 `gateway.auth`。Doctor 可以随时为你生成令牌：`OpenKrab doctor --generate-gateway-token`。
...
`web_fetch` 无需 API 密钥即可工作。`web_search` 需要 Brave Search API 密钥。**推荐：** 运行 `OpenKrab configure --section web` 将其存储在 `tools.web.search.apiKey` 中。环境变量替代方案：为 Gateway 网关进程设置 `BRAVE_API_KEY`。
...
- 守护进程从 `~/.OpenKrab/.env`（或服务环境）读取环境变量。
...
- 从备份恢复（git 或复制的 `~/.OpenKrab/OpenKrab.json`）。
- 如果没有备份，重新运行 `OpenKrab doctor` 并重新配置渠道/模型。
- 如果这是意外情况，提交 bug 并附上你最后已知的配置或任何备份。
- 本地编码智能体通常可以从日志或历史中重建工作配置。

避免方法：

- 对小更改使用 `OpenKrab config set`。
- 对交互式编辑使用 `OpenKrab configure`。
...
- Gateway 网关正在运行：`OpenKrab gateway status`
- Gateway 网关健康：`OpenKrab status`
- 渠道健康：`OpenKrab channels status`
...
**CLI 桥接（通用）：** 运行一个脚本调用另一个 Gateway 网关，使用 `OpenKrab agent --message ... --deliver`，定向到另一个机器人监听的聊天。如果一个机器人在远程 VPS 上，通过 SSH/Tailscale 将你的 CLI 指向该远程 Gateway 网关（参阅[远程访问](/gateway/remote)）。
...
- 当前工作目录下的 `.env`
- `~/.OpenKrab/.env`（即 `$OpenKrab_STATE_DIR/.env`）的全局回退 `.env`
...
1. 将缺失的密钥放在 `~/.OpenKrab/.env` 中，这样即使服务不继承你的 shell 环境也能被获取。
...
`OpenKrab models status` 报告的是 **shell 环境导入**是否启用。"Shell env: off"**不**意味着你的环境变量缺失——它只意味着 OpenKrab 不会自动加载你的登录 shell。

如果 Gateway 网关作为服务（launchd/systemd）运行，它不会继承你的 shell 环境。通过以下方式之一修复：

1. 将令牌放在 `~/.OpenKrab/.env` 中：
   ```
...
- 如果你使用了配置文件（`--profile` / `OpenKrab_PROFILE`），重置每个状态目录（默认为 `~/.OpenKrab-<profile>`）。
- 开发重置：`OpenKrab gateway --dev --reset`（仅限开发；清除开发配置 + 凭据 + 会话 + 工作区）。
...
- **磁盘增长：** 会话 + 记录位于 `~/.OpenKrab/agents/<agentId>/sessions/` 下。
...
- 使用 `OpenKrab doctor` 发现无用的工作区和配置文件不匹配。
...
- 聊天中的 `/model`（快速，按会话）
- `OpenKrab models set ...`（只更新模型配置）
- `OpenKrab configure --section models`（交互式）
- 编辑 `~/.OpenKrab/OpenKrab.json` 中的 `agents.defaults.model`

避免使用部分对象执行 `config.apply`，除非你打算替换整个配置。如果你确实覆盖了配置，从备份恢复或重新运行 `OpenKrab doctor` 来修复。
...
- 运行 `OpenKrab agents add <id>` 并在向导中配置认证。
...
- **确认认证配置文件的位置**（新路径 vs 旧路径）
  - 当前：`~/.OpenKrab/agents/<agentId>/agent/auth-profiles.json`
  - 旧版：`~/.OpenKrab/agent/*`（通过 `OpenKrab doctor` 迁移）
- **确认环境变量被 Gateway 网关加载**
  - 如果你在 shell 中设置了 `ANTHROPIC_API_KEY` 但通过 systemd/launchd 运行 Gateway 网关，它可能不会继承。将其放在 `~/.OpenKrab/.env` 中或启用 `env.shellEnv`。
- **确保你编辑的是正确的智能体**
  - 多智能体设置意味着可能有多个 `auth-profiles.json` 文件。
- **完整性检查模型/认证状态**
  - 使用 `OpenKrab models status` 查看已配置的模型以及提供商是否已认证。
...
- **使用 setup-token**
  - 运行 `claude setup-token`，然后用 `OpenKrab models auth setup-token --provider anthropic` 粘贴。
  - 如果令牌在另一台机器上创建，使用 `OpenKrab models auth paste-token --provider anthropic`。
- **如果你想使用 API 密钥**
  - 在 **Gateway 网关主机**上将 `ANTHROPIC_API_KEY` 放入 `~/.OpenKrab/.env`。
  - 清除任何强制缺失配置文件的固定顺序：
    ```bash
...
如果某个配置文件处于短期**冷却**（速率限制/超时/认证失败）或较长的**禁用**状态（计费/额度不足），OpenKrab 可能会临时跳过它。要检查这一点，运行 `OpenKrab models status --json` 并查看 `auth.unusableProfiles`。调优：`auth.cooldowns.billingBackoffHours*`。
...
```
--port > OpenKrab_GATEWAY_PORT > gateway.port > 默认 18789
```

### 为什么 OpenKrab gateway status 显示 Runtime: running 但 RPC probe: failed

因为"running"是 **supervisor** 的视角（launchd/systemd/schtasks）。RPC 探测是 CLI 实际连接到 Gateway 网关 WebSocket 并调用 `status`。

使用 `OpenKrab gateway status` 并关注这些行：

- `Probe target:`（探测实际使用的 URL）
- `Listening:`（端口上实际绑定的内容）
- `Last gateway error:`（进程存活但端口未监听时的常见根因）

### 为什么 OpenKrab gateway status 显示 Config (cli) 和 Config (service) 不同

你正在编辑一个配置文件，而服务运行的是另一个（通常是 `--profile` / `OpenKrab_STATE_DIR` 不匹配）。
...
修复：停止另一个实例，释放端口，或使用 `OpenKrab gateway --port <port>` 运行。
...
- `OpenKrab gateway` 仅在 `gateway.mode` 为 `local` 时启动（或你传递覆盖标志）。
...
- 控制 UI 将令牌存储在浏览器 localStorage 键 `OpenKrab.control.settings.v1` 中。
- UI 可以导入一次 `?token=...`（和/或 `?password=...`），然后从 URL 中剥离。

修复：

- 最快：`OpenKrab dashboard`（打印 + 复制带令牌的链接，尝试打开；如果无头则显示 SSH 提示）。
- 如果你还没有令牌：`OpenKrab doctor --generate-gateway-token`。
- 如果是远程，先建隧道：`ssh -N -L 18789:127.0.0.1:18789 user@host` 然后打开 `http://127.0.0.1:18789/?token=...`。
- 在 Gateway 网关主机上设置 `gateway.auth.token`（或 `OpenKrab_GATEWAY_TOKEN`）。
- 在控制 UI 设置中粘贴相同的令牌（或使用一次性 `?token=...` 链接刷新）。
- 仍然卡住？运行 `OpenKrab status --all` 并按[故障排除](/gateway/troubleshooting)操作。参阅[仪表板](/web/dashboard)了解认证详情。
...
- `OpenKrab_CONFIG_PATH`（每实例配置）
- `OpenKrab_STATE_DIR`（每实例状态）
- `agents.defaults.workspace`（工作区隔离）
- `gateway.port`（唯一端口）

快速设置（推荐）：

- 每实例使用 `OpenKrab --profile <name> …`（自动创建 `~/.OpenKrab-<name>`）。
- 在每个配置文件配置中设置唯一的 `gateway.port`（或手动运行时传 `--port`）。
- 安装每配置文件的服务：`OpenKrab --profile <name> gateway install`。

配置文件还会为服务名称添加后缀（`bot.molt.<profile>`；旧版 `com.OpenKrab.*`、`OpenKrab-gateway-<profile>.service`、`OpenKrab Gateway 网关 (<profile>)`）。
...
- macOS：`$OpenKrab_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.OpenKrab/logs/...`；配置文件使用 `~/.OpenKrab-<profile>/logs/...`）
...
如果你手动运行 Gateway 网关，`OpenKrab gateway --force` 可以回收端口。参阅 [Gateway 网关](/gateway)。
...
1. Gateway 网关在运行吗？`OpenKrab gateway status`
2. Gateway 网关健康吗？`OpenKrab status`
3. UI 有正确的令牌吗？`OpenKrab dashboard`
...
### 通俗解释：OpenKrab gateway restart 与 OpenKrab gateway

- `OpenKrab gateway restart`：重启**后台服务**（launchd/systemd）。
- `OpenKrab gateway`：在这个终端会话中**前台**运行 Gateway 网关。

如果你安装了服务，使用 Gateway 网关命令。想要一次性前台运行时使用 `OpenKrab gateway`。
...
智能体的出站附件必须包含 `MEDIA:<path-or-url>` 行（独占一行）。参阅 [OpenKrab 助手设置](/start/OpenKrab)和 [Agent send](/tools/agent-send)。
...
- 支持私信的渠道上的默认行为是**配对**：
  ...
  - 批准方式：`OpenKrab pairing approve <channel> <code>`
  - 每个渠道的待处理请求上限为 **3 个**；如果没收到代码，检查 `OpenKrab pairing list <channel>`。
- 公开开放私信需要明确选择加入（`dmPolicy: "open"` 且允许列表 `"*"`）。

运行 `OpenKrab doctor` 以发现有风险的私信策略。
...
仍然卡住？在 [Discord](https://discord.com/invite/clawd) 中提问或发起 [GitHub 讨论](https://github.com/OpenKrab/OpenKrab/discussions)。
