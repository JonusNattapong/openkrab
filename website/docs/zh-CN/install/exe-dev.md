...
提示：保持此 VM **有状态**。OpenKrab 在 `~/.OpenKrab/` 和 `~/.OpenKrab/workspace/` 下存储状态。
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`（参阅新手引导中的控制 UI 输出）。使用 `OpenKrab devices list` 和 `OpenKrab devices approve <requestId>` 批准设备。如有疑问，从浏览器使用 Shelley！
