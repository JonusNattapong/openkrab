---
read_when:
  - ä½ æ­£åœ¨å°† OpenKrab è¿ç§»åˆ°æ–°çš„ç¬”è®°æœ¬ç”µè„‘/æœåŠ¡å™¨
  - ä½ æƒ³ä¿ç•™ä¼šè¯ã€è®¤è¯å’Œæ¸ é“ç™»å½•ï¼ˆWhatsApp ç­‰ï¼‰
summary: å°† OpenKrab å®‰è£…ä»Žä¸€å°æœºå™¨è¿ç§»åˆ°å¦ä¸€å°
title: è¿ç§»æŒ‡å—
x-i18n:
...
# å°† OpenKrab è¿ç§»åˆ°æ–°æœºå™¨

æœ¬æŒ‡å—å°† OpenKrab Gateway ç½‘å…³ä»Žä¸€å°æœºå™¨è¿ç§»åˆ°å¦ä¸€å°ï¼Œ**æ— éœ€é‡æ–°è¿›è¡Œæ–°æ‰‹å¼•å¯¼**ã€‚

è¿ç§»åœ¨æ¦‚å¿µä¸Šå¾ˆç®€å•ï¼š

- å¤åˆ¶**çŠ¶æ€ç›®å½•**ï¼ˆ`$OPENKRAB_STATE_DIR`ï¼Œé»˜è®¤ï¼š`~/.OpenKrab/`ï¼‰â€” è¿™åŒ…æ‹¬é…ç½®ã€è®¤è¯ã€ä¼šè¯å’Œæ¸ é“çŠ¶æ€ã€‚
- å¤åˆ¶ä½ çš„**å·¥ä½œåŒº**ï¼ˆé»˜è®¤ `~/.OpenKrab/workspace/`ï¼‰â€” è¿™åŒ…æ‹¬ä½ çš„æ™ºèƒ½ä½“æ–‡ä»¶ï¼ˆè®°å¿†ã€æç¤ºç­‰ï¼‰ã€‚
...
- **çŠ¶æ€ç›®å½•ï¼š** `~/.OpenKrab/`

ä½†å¦‚æžœä½ ä½¿ç”¨ä»¥ä¸‹æ–¹å¼ï¼Œå¯èƒ½ä¼šä¸åŒï¼š

- `--profile <name>`ï¼ˆé€šå¸¸å˜æˆ `~/.OpenKrab-<profile>/`ï¼‰
...
åœ¨è¾“å‡ºä¸­æŸ¥æ‰¾ `OPENKRAB_STATE_DIR` / profile çš„æåŠã€‚å¦‚æžœä½ è¿è¡Œå¤šä¸ª Gateway ç½‘å…³ï¼Œå¯¹æ¯ä¸ªé…ç½®æ–‡ä»¶é‡å¤æ­¤æ“ä½œã€‚

### 2ï¼‰ç¡®å®šä½ çš„å·¥ä½œåŒº

å¸¸è§é»˜è®¤å€¼ï¼š

- `~/.OpenKrab/workspace/`ï¼ˆæŽ¨èçš„å·¥ä½œåŒºï¼‰
...
- Gateway ç½‘å…³é…ç½®ï¼ˆ`OpenKrab.json`ï¼‰
...
è¿™äº›å­˜å‚¨åœ¨ `$OPENKRAB_STATE_DIR` ä¸‹ã€‚
...
å¦‚æžœä½ æœ‰å¤šä¸ªé…ç½®æ–‡ä»¶/çŠ¶æ€ç›®å½•ï¼ˆä¾‹å¦‚ `~/.OpenKrab-main`ã€`~/.OpenKrab-work`ï¼‰ï¼Œåˆ†åˆ«å½’æ¡£æ¯ä¸ªã€‚

### æ­¥éª¤ 1 â€” åœ¨æ–°æœºå™¨ä¸Šå®‰è£… OpenKrab

åœ¨**æ–°**æœºå™¨ä¸Šï¼Œå®‰è£… CLIï¼ˆå¦‚æžœéœ€è¦è¿˜æœ‰ Nodeï¼‰ï¼š

- å‚è§ï¼š[å®‰è£…](/install)

åœ¨è¿™ä¸ªé˜¶æ®µï¼Œå¦‚æžœæ–°æ‰‹å¼•å¯¼åˆ›å»ºäº†ä¸€ä¸ªæ–°çš„ `~/.OpenKrab/` ä¹Ÿæ²¡å…³ç³» â€” ä½ å°†åœ¨ä¸‹ä¸€æ­¥è¦†ç›–å®ƒã€‚

### æ­¥éª¤ 2 â€” å°†çŠ¶æ€ç›®å½• + å·¥ä½œåŒºå¤åˆ¶åˆ°æ–°æœºå™¨

å¤åˆ¶**ä¸¤è€…**ï¼š

- `$OPENKRAB_STATE_DIR`ï¼ˆé»˜è®¤ `~/.OpenKrab/`ï¼‰
- ä½ çš„å·¥ä½œåŒºï¼ˆé»˜è®¤ `~/.OpenKrab/workspace/`ï¼‰
...
- åŒ…å«äº†éšè—ç›®å½•ï¼ˆä¾‹å¦‚ `.OpenKrab/`ï¼‰
...
å¦‚æžœä½ åœ¨æ—§ Gateway ç½‘å…³ä¸Šä½¿ç”¨äº†é…ç½®æ–‡ä»¶ï¼ˆæˆ– `OPENKRAB_STATE_DIR`ï¼‰ï¼Œè€Œæ–° Gateway ç½‘å…³ä½¿ç”¨äº†ä¸åŒçš„é…ç½®ï¼Œä½ ä¼šçœ‹åˆ°å¦‚ä¸‹ç—‡çŠ¶ï¼š
...
### é™·é˜±ï¼šåªå¤åˆ¶ `OpenKrab.json`

`OpenKrab.json` æ˜¯ä¸å¤Ÿçš„ã€‚è®¸å¤šæä¾›å•†åœ¨ä»¥ä¸‹ä½ç½®å­˜å‚¨çŠ¶æ€ï¼š

- `$OPENKRAB_STATE_DIR/credentials/`
- `$OPENKRAB_STATE_DIR/agents/<agentId>/...`

å§‹ç»ˆè¿ç§»æ•´ä¸ª `$OPENKRAB_STATE_DIR` æ–‡ä»¶å¤¹ã€‚
...
`$OPENKRAB_STATE_DIR` åŒ…å«å¯†é’¥ï¼ˆAPI å¯†é’¥ã€OAuth ä»¤ç‰Œã€WhatsApp å‡­è¯ï¼‰ã€‚å°†å¤‡ä»½è§†ä¸ºç”Ÿäº§å¯†é’¥ï¼š
...
- `OpenKrab status` æ˜¾ç¤º Gateway ç½‘å…³æ­£åœ¨è¿è¡Œ
...
- [OpenKrab åœ¨å“ªé‡Œå­˜å‚¨æ•°æ®ï¼Ÿ](/help/faq#where-does-OpenKrab-store-its-data)
