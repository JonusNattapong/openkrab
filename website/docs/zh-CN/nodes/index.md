...
macOS ä¹Ÿå¯ä»¥åœ¨**èŠ‚ç‚¹æ¨¡å¼**ä¸‹è¿è¡Œï¼šèœå•æ åº”ç”¨è¿žæŽ¥åˆ° Gateway ç½‘å…³çš„ WS æœåŠ¡å™¨ï¼Œå¹¶å°†å…¶æœ¬åœ° canvas/camera å‘½ä»¤ä½œä¸ºèŠ‚ç‚¹æš´éœ²ï¼ˆå› æ­¤ `OpenKrab nodes â€¦` å¯ä»¥é’ˆå¯¹è¿™å° Mac å·¥ä½œï¼‰ã€‚
...
- `node.pair.*`ï¼ˆCLIï¼š`OpenKrab nodes pending/approve/reject`ï¼‰æ˜¯ä¸€ä¸ªå•ç‹¬çš„ Gateway ç½‘å…³æ‹¥æœ‰çš„
...
- **æ‰¹å‡†**ï¼šé€šè¿‡ `~/.OpenKrab/exec-approvals.json` åœ¨èŠ‚ç‚¹ä¸»æœºä¸Šæ‰§è¡Œã€‚
...
```bash
OpenKrab node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
- ä»¤ç‰Œæ˜¯ Gateway ç½‘å…³é…ç½®ä¸­çš„ `gateway.auth.token`ï¼ˆGateway ç½‘å…³ä¸»æœºä¸Šçš„ `~/.OpenKrab/OpenKrab.json`ï¼‰ã€‚
- `OpenKrab node run` è¯»å– `OPENKRAB_GATEWAY_TOKEN` è¿›è¡Œè®¤è¯ã€‚

### å¯åŠ¨èŠ‚ç‚¹ä¸»æœºï¼ˆæœåŠ¡ï¼‰

```bash
OpenKrab node install --host <gateway-host> --port 18789 --display-name "Build Node"
...
- åœ¨ `OpenKrab node run` / `OpenKrab node install` ä¸Šä½¿ç”¨ `--display-name`ï¼ˆæŒä¹…åŒ–åœ¨èŠ‚ç‚¹ä¸Šçš„ `~/.OpenKrab/node.json` ä¸­ï¼‰ã€‚
- `OpenKrab nodes rename --node <id|name|ip> --name "Build Node"`ï¼ˆGateway ç½‘å…³è¦†ç›–ï¼‰ã€‚
...
```bash
OpenKrab approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
OpenKrab approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

æ‰¹å‡†å­˜å‚¨åœ¨èŠ‚ç‚¹ä¸»æœºçš„ `~/.OpenKrab/exec-approvals.json` ä¸­ã€‚
...
```bash
OpenKrab config set tools.exec.host node
OpenKrab config set tools.exec.security allowlist
OpenKrab config set tools.exec.node "<id-or-name>"
```
...
```bash
OpenKrab nodes canvas snapshot --node <idOrNameOrIp> --format png
OpenKrab nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas æŽ§åˆ¶

```bash
OpenKrab nodes canvas present --node <idOrNameOrIp> --target https://example.com
OpenKrab nodes canvas hide --node <idOrNameOrIp>
OpenKrab nodes canvas navigate https://example.com --node <idOrNameOrIp>
OpenKrab nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
OpenKrab nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
OpenKrab nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
OpenKrab nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
OpenKrab nodes camera list --node <idOrNameOrIp>
OpenKrab nodes camera snap --node <idOrNameOrIp>            # é»˜è®¤ï¼šä¸¤ä¸ªæœå‘ï¼ˆ2 ä¸ª MEDIA è¡Œï¼‰
OpenKrab nodes camera snap --node <idOrNameOrIp> --facing front
```

è§†é¢‘ç‰‡æ®µï¼ˆ`mp4`ï¼‰ï¼š

```bash
OpenKrab nodes camera clip --node <idOrNameOrIp> --duration 10s
OpenKrab nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
OpenKrab nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
OpenKrab nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
OpenKrab nodes location get --node <idOrNameOrIp>
OpenKrab nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
OpenKrab nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from OpenKrab"}'
```
...
```bash
OpenKrab nodes run --node <idOrNameOrIp> -- echo "Hello from mac node"
OpenKrab nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
```
...
- åœ¨æ— å¤´èŠ‚ç‚¹ä¸»æœºä¸Šï¼Œ`system.run` å— exec æ‰¹å‡†é™åˆ¶ï¼ˆ`~/.OpenKrab/exec-approvals.json`ï¼‰ã€‚
...
```bash
OpenKrab config set tools.exec.node "node-id-or-name"
```
...
```bash
OpenKrab config unset tools.exec.node
OpenKrab config unset agents.list[0].tools.exec.node
```
...
OpenKrab å¯ä»¥è¿è¡Œ**æ— å¤´èŠ‚ç‚¹ä¸»æœº**ï¼ˆæ—  UIï¼‰ï¼Œå®ƒè¿žæŽ¥åˆ° Gateway ç½‘å…³
...
- èŠ‚ç‚¹ä¸»æœºå°†å…¶èŠ‚ç‚¹ idã€ä»¤ç‰Œã€æ˜¾ç¤ºåç§°å’Œ Gateway ç½‘å…³è¿žæŽ¥ä¿¡æ¯å­˜å‚¨åœ¨ `~/.OpenKrab/node.json` ä¸­ã€‚
- Exec æ‰¹å‡†é€šè¿‡ `~/.OpenKrab/exec-approvals.json` åœ¨æœ¬åœ°æ‰§è¡Œ
  ï¼ˆå‚è§ [Exec æ‰¹å‡†](/tools/exec-approvals)ï¼‰ã€‚
- åœ¨ macOS ä¸Šï¼Œå½“é…å¥—åº”ç”¨ exec ä¸»æœºå¯è¾¾æ—¶ï¼Œæ— å¤´èŠ‚ç‚¹ä¸»æœºä¼˜å…ˆä½¿ç”¨å®ƒï¼Œ
  å¦‚æžœåº”ç”¨ä¸å¯ç”¨åˆ™å›žé€€åˆ°æœ¬åœ°æ‰§è¡Œã€‚è®¾ç½® `OPENKRAB_NODE_EXEC_HOST=app` è¦æ±‚
  ä½¿ç”¨åº”ç”¨ï¼Œæˆ–è®¾ç½® `OPENKRAB_NODE_EXEC_FALLBACK=0` ç¦ç”¨å›žé€€ã€‚
- å½“ Gateway ç½‘å…³ WS ä½¿ç”¨ TLS æ—¶ï¼Œæ·»åŠ  `--tls` / `--tls-fingerprint`ã€‚

## Mac èŠ‚ç‚¹æ¨¡å¼

- macOS èœå•æ åº”ç”¨ä½œä¸ºèŠ‚ç‚¹è¿žæŽ¥åˆ° Gateway ç½‘å…³ WS æœåŠ¡å™¨ï¼ˆå› æ­¤ `OpenKrab nodes â€¦` å¯ä»¥é’ˆå¯¹è¿™å° Mac å·¥ä½œï¼‰ã€‚
