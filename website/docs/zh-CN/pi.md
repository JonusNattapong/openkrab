...
æœ¬æ–‡æ¡£æè¿°äº† OpenKrab å¦‚ä½•ä¸Ž [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) åŠå…¶ç›¸å…³åŒ…ï¼ˆ`pi-ai`ã€`pi-agent-core`ã€`pi-tui`ï¼‰é›†æˆä»¥å®žçŽ°å…¶ AI æ™ºèƒ½ä½“èƒ½åŠ›ã€‚

## æ¦‚è¿°

OpenKrab ä½¿ç”¨ pi SDK å°† AI ç¼–ç æ™ºèƒ½ä½“åµŒå…¥åˆ°å…¶æ¶ˆæ¯ Gateway ç½‘å…³æž¶æž„ä¸­ã€‚OpenKrab ä¸æ˜¯å°† pi ä½œä¸ºå­è¿›ç¨‹ç”Ÿæˆæˆ–ä½¿ç”¨ RPC æ¨¡å¼ï¼Œè€Œæ˜¯é€šè¿‡ `createAgentSession()` ç›´æŽ¥å¯¼å…¥å¹¶å®žä¾‹åŒ– pi çš„ `AgentSession`ã€‚è¿™ç§åµŒå…¥å¼æ–¹æ³•æä¾›äº†ï¼š
...
| `pi-tui`          | ç»ˆç«¯ UI ç»„ä»¶ï¼ˆç”¨äºŽ OpenKrab çš„æœ¬åœ° TUI æ¨¡å¼ï¼‰                                              |
...
â”œâ”€â”€ pi-tools.ts                    # createOpenKrabCodingTools()
...
â”œâ”€â”€ OpenKrab-tools.ts              # OpenKrab-specific tools
...
2. **è‡ªå®šä¹‰æ›¿æ¢**ï¼šOpenKrab å°† bash æ›¿æ¢ä¸º `exec`/`process`ï¼Œä¸ºæ²™ç®±è‡ªå®šä¹‰ read/edit/write
3. **OpenKrab å·¥å…·**ï¼šæ¶ˆæ¯ã€æµè§ˆå™¨ã€ç”»å¸ƒã€ä¼šè¯ã€å®šæ—¶ä»»åŠ¡ã€Gateway ç½‘å…³ç­‰
...
è¿™ç¡®ä¿ OpenKrab çš„ç­–ç•¥è¿‡æ»¤ã€æ²™ç®±é›†æˆå’Œæ‰©å±•å·¥å…·é›†åœ¨å„æä¾›å•†ä¹‹é—´ä¿æŒä¸€è‡´ã€‚

## ç³»ç»Ÿæç¤ºæž„å»º

ç³»ç»Ÿæç¤ºåœ¨ `buildAgentSystemPrompt()`ï¼ˆ`system-prompt.ts`ï¼‰ä¸­æž„å»ºã€‚å®ƒç»„è£…ä¸€ä¸ªå®Œæ•´çš„æç¤ºï¼ŒåŒ…å«å·¥å…·ã€å·¥å…·è°ƒç”¨é£Žæ ¼ã€å®‰å…¨æŠ¤æ ã€OpenKrab CLI å‚è€ƒã€Skillsã€æ–‡æ¡£ã€å·¥ä½œåŒºã€æ²™ç®±ã€æ¶ˆæ¯ã€å›žå¤æ ‡ç­¾ã€è¯­éŸ³ã€é™é»˜å›žå¤ã€å¿ƒè·³ã€è¿è¡Œæ—¶å…ƒæ•°æ®ç­‰éƒ¨åˆ†ï¼Œä»¥åŠå¯ç”¨æ—¶çš„è®°å¿†å’Œååº”ï¼Œè¿˜æœ‰å¯é€‰çš„ä¸Šä¸‹æ–‡æ–‡ä»¶å’Œé¢å¤–ç³»ç»Ÿæç¤ºå†…å®¹ã€‚éƒ¨åˆ†å†…å®¹åœ¨å­æ™ºèƒ½ä½“ä½¿ç”¨çš„æœ€å°æç¤ºæ¨¡å¼ä¸‹ä¼šè¢«è£å‰ªã€‚
...
OpenKrab ç”¨ `guardSessionManager()` åŒ…è£…å®ƒä»¥ç¡®ä¿å·¥å…·ç»“æžœå®‰å…¨ã€‚
...
OpenKrab ç»´æŠ¤ä¸€ä¸ªè®¤è¯é…ç½®æ–‡ä»¶å­˜å‚¨ï¼Œæ¯ä¸ªæä¾›å•†æœ‰å¤šä¸ª API å¯†é’¥ï¼š
...
OpenKrab åŠ è½½è‡ªå®šä¹‰ pi æ‰©å±•ä»¥å®žçŽ°ç‰¹æ®Šè¡Œä¸ºï¼š
...
OpenKrab è¿˜æœ‰ä¸€ä¸ªæœ¬åœ° TUI æ¨¡å¼ï¼Œç›´æŽ¥ä½¿ç”¨ pi-tui ç»„ä»¶ï¼š
...
| æ–¹é¢     | Pi CLI                  | OpenKrab åµŒå…¥å¼                                                                                 |
| -------- | ----------------------- | ----------------------------------------------------------------------------------------------- |
| è°ƒç”¨æ–¹å¼ | `pi` å‘½ä»¤ / RPC         | é€šè¿‡ `createAgentSession()` çš„ SDK                                                              |
| å·¥å…·     | é»˜è®¤ç¼–ç å·¥å…·            | è‡ªå®šä¹‰ OpenKrab å·¥å…·å¥—ä»¶                                                                        |
| ç³»ç»Ÿæç¤º | AGENTS.md + prompts     | æŒ‰æ¸ é“/ä¸Šä¸‹æ–‡åŠ¨æ€ç”Ÿæˆ                                                                           |
| ä¼šè¯å­˜å‚¨ | `~/.pi/agent/sessions/` | `~/.OpenKrab/agents/<agentId>/sessions/`ï¼ˆæˆ– `$OPENKRAB_STATE_DIR/agents/<agentId>/sessions/`ï¼‰ |
...
- `src/agents/pi-tools.create-OpenKrab-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-OpenKrab-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-OpenKrab-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-OpenKrab-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
