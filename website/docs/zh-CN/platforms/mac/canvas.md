...
```bash
OpenKrab nodes canvas present --node <id>
OpenKrab nodes canvas navigate --node <id> --url "/"
OpenKrab nodes canvas eval --node <id> --js "document.title"
OpenKrab nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__OpenKrab__/a2ui/
```
...
OpenKrab nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
OpenKrab nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "OpenKrab://agent?message=Review%20this%20design";
```
