...
- 要求来源于每个 `SKILL.md` 中的 `metadata.OpenKrab.requires`。

## 安装操作

- `metadata.OpenKrab.install` 定义安装选项（brew/node/go/uv）。
...
- 应用将密钥存储在 `~/.OpenKrab/OpenKrab.json` 的 `skills.entries.<skillKey>` 下。
