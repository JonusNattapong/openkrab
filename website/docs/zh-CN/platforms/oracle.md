...
这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh OpenKrab` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@OpenKrab`（或使用 Tailscale IP）。
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `OpenKrab security audit`，并验证你没有意外地在公共接口上监听。
...
- **凭证权限：** `chmod 700 ~/.OpenKrab`
- **安全审计：** `OpenKrab security audit`
...
- `~/.OpenKrab/` — 配置、凭证、会话数据
- `~/.OpenKrab/workspace/` — 工作区（SOUL.md、记忆、产物）
