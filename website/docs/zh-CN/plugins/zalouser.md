...
```bash
OpenKrab plugins install @OpenKrab/zalouser
```
...
```bash
OpenKrab plugins install ./extensions/zalouser
...
OpenKrab channels logout --channel zalouser
OpenKrab channels status --probe
OpenKrab message send --channel zalouser --target <threadId> --message "Hello from OpenKrab"
