...
```bash
OpenKrab plugins enable minimax-portal-auth  # 如果已加载则跳过
OpenKrab gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/OpenKrab/OpenKrab/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `OpenKrab models list` 和 `OpenKrab models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `OpenKrab configure` 并选择 **MiniMax M2.1**，或
