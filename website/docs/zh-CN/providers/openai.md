...
```bash
OpenKrab onboard --auth-choice openai-api-key
# 或非交互式
OpenKrab onboard --openai-api-key "$OPENAI_API_KEY"
```
...
```bash
# 在向导中运行 Codex OAuth
OpenKrab onboard --auth-choice openai-codex

# 或直接运行 OAuth
OpenKrab models auth login --provider openai-codex
```
