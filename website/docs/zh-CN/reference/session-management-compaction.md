...
- 存储：`~/.OpenKrab/agents/<agentId>/sessions/sessions.json`
- 记录：`~/.OpenKrab/agents/<agentId>/sessions/<sessionId>.jsonl`
...
- 存储 vs 记录不匹配？从 `OpenKrab status` 确认 Gateway 网关主机和存储路径。
- 压缩过于频繁？检查：
  - 模型上下文窗口（太小）
