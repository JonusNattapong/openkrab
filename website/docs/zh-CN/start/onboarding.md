...
- 打开浏览器进行 OAuth（PKCE）
- 要求用户粘贴 `code#state` 值
- 将凭证写入 `~/.OpenKrab/credentials/oauth.json`
...
应用可以通过 npm/pnpm 安装全局 `OpenKrab` CLI，以便终端工作流和 launchd 任务开箱即用。
...
在首次智能体运行时，OpenKrab 会引导一个工作区（默认 `~/.OpenKrab/workspace`）：
...
```bash
OpenKrab webhooks gmail setup --account you@gmail.com
```
...
- `~/.OpenKrab/credentials/oauth.json`
