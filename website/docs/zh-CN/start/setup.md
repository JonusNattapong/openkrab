...
- **个性化设置存放在仓库之外：** `~/.OpenKrab/workspace`（工作区）+ `~/.OpenKrab/OpenKrab.json`（配置）。
...
- **配置：** `~/.OpenKrab/OpenKrab.json`（JSON/JSON5 格式）
- **工作区：** `~/.OpenKrab/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm OpenKrab setup` 运行它。
...
- 运行 `OpenKrab setup`，然后 `OpenKrab channels login`，然后手动启动 Gateway 网关（`OpenKrab gateway`）。
...
- **状态存储位置：**
  ...
  - 会话：`~/.OpenKrab/agents/<agentId>/sessions/`
  - 日志：`/tmp/OpenKrab/`
...
- **配对允许列表**：`~/.OpenKrab/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.OpenKrab/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.OpenKrab/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.OpenKrab/workspace` 和 `~/.OpenKrab/` 保持为"你的东西"；不要将个人提示/配置放入 `OpenKrab` 仓库。
