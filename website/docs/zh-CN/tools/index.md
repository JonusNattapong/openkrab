...
read_when:
  - 添加或修改智能体工具
  - 停用或更改 `OpenKrab-*` Skills
summary: OpenKrab 的智能体工具接口（browser、canvas、nodes、message、cron），替代旧版 `OpenKrab-*` Skills
title: 工具
x-i18n:
...
这些工具取代了旧的 `OpenKrab-*` Skills：工具是类型化的，无需调用 shell，
智能体应该直接依赖它们。

## 禁用工具

你可以通过 `OpenKrab.json` 中的 `tools.allow` / `tools.deny` 全局允许/拒绝工具
...
- `host=node` 可以针对 macOS 配套应用或无头节点主机（`OpenKrab node run`）。
...
- 需要 Brave API 密钥（推荐：`OpenKrab configure --section web`，或设置 `BRAVE_API_KEY`）。
...
- 快速冒烟测试：`OpenKrab nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
...
- `restart`（授权 + 发送 `SIGUSR1` 进行进程内重启；`OpenKrab gateway` 就地重启）
